...
class TestConfig:
    ...
    def __init__(self, env, name: str):
        ...
        self.tests = TestCollection(
            env, getattr(config, "tests", ["*"]), getattr(config, "categories", ["*"])
        )
    ...
    def _update_queue(self, env) -> None:
        ...
        for revision_name, revision_commit in self.revisions.items():
            revision_commit, environment = self._split_environment_variables(
                revision_commit
            )
        ...
        for revision_name, executable in self.builds.items():
            ...
            executable_path = env._blender_executable_from_path(
            ...
            entries += self._get_entries(
                revision_name, git_hash, executable, environment, mtime
            )
    ...
    def _get_entries(
        ...
        for test in self.tests.tests:
            ...
            for device in self.devices:
                entry = self.queue.find(
                    revision_name, test_name, test_category, device.id
                )
                if entry:
                    ...
                    if (
                        ...
                        or entry.environment != environment
                        or entry.benchmark_type != self.benchmark_type
                        or entry.date != date
                    ):
