...
class TestEnvironment:
    ...
    def init(self, build) -> None:
        if not self.benchmarks_dir.exists():
            sys.stderr.write(
                f"Error: benchmark files directory not found at {self.benchmarks_dir}"
            )
            sys.exit(1)
        ...
        if build:
            ...
            if not self.build_dir.exists():
                ...
                self.call(
                    [self.cmake_executable, self.blender_dir, "."] + self.cmake_options,
                    self.build_dir,
                )
            else:
    ...
    def checkout(self, git_hash) -> None:
        ...
        self.call(
            [self.git_executable, "checkout", "--detach", git_hash], self.blender_dir
        )
    ...
    def set_blender_executable(
        self, executable_path: pathlib.Path, environment: Dict = None
    ) -> None:
    ...
    def call_blender(self, args: List[str], foreground=False) -> List[str]:
        ...
        return self.call(
            [self.blender_executable] + common_args + args,
            cwd=self.base_dir,
    ...
    def resolve_git_hash(self, revision):
        ...
        lines = self.call(
            [self.git_executable, "rev-parse", revision], self.blender_git_dir
        )
    ...
    def git_hash_date(self, git_hash):
        ...
        lines = self.call(
            [self.git_executable, "log", "-n1", git_hash, "--format=%at"],
            self.blender_git_dir,
        )
