...
class AbstractAlembicTest(unittest.TestCase):
    ...
    def setUp(self):
        self.assertTrue(
            self.testdir.exists(), "Test dir %s should exist" % self.testdir
        )
...
class SimpleImportTest(AbstractAlembicTest):
    def test_import_cube_hierarchy(self):
        res = bpy.ops.wm.alembic_import(
            filepath=str(self.testdir / "cubes-hierarchy.abc"), as_background_job=False
        )
    ...
    def test_import_long_names(self):
        ...
        bpy.ops.wm.alembic_import(
            filepath=str(self.testdir / "long-names.abc"), as_background_job=False
        )
...
class VertexColourImportTest(AbstractAlembicTest):
    def test_import_from_houdini(self):
        ...
        self.assertAlmostEqualFloatArray(
            layer.data[98].color, (0.9019607, 0.4745098, 0.2666666, 1.0)
        )
        self.assertAlmostEqualFloatArray(
            layer.data[99].color, (0.8941176, 0.4705882, 0.2627451, 1.0)
        )
    ...
    def test_import_from_blender(self):
        ...
        # Test some known-good values.
        self.assertAlmostEqualFloatArray(
            layer.data[0].color, (1.0, 0.0156862, 0.3607843, 1.0)
        )
        self.assertAlmostEqualFloatArray(
            layer.data[98].color, (0.0941176, 0.1215686, 0.9137254, 1.0)
        )
        self.assertAlmostEqualFloatArray(
            layer.data[99].color, (0.1294117, 0.3529411, 0.7529411, 1.0)
        )
...
class CameraExportImportTest(unittest.TestCase):
    ...
    def do_export_import_test(self, *, flatten: bool):
        ...
        self.loc_rot_scale(
            "CAM_Animated_Child_Left", (3.864053, 0.936607, 2), (90, 0, 25)
        )
        self.loc_rot_scale(
            "CAM_Animated_Child_Right", (4.135946, 1.063392, 2), (90, 0, 25)
        )
    ...
    def loc_rot_scale(self, name: str, expect_loc, expect_rot_deg):
        ...
        self.assertAlmostEqual(
            expect_rot_deg[0], math.degrees(actual_rot.x), delta=delta_rot
        )
        self.assertAlmostEqual(
            expect_rot_deg[1], math.degrees(actual_rot.y), delta=delta_rot
        )
        self.assertAlmostEqual(
            expect_rot_deg[2], math.degrees(actual_rot.z), delta=delta_rot
        )
...
class OverrideLayersTest(AbstractAlembicTest):
    def test_import_layer(self):
        ...
        res = bpy.ops.wm.alembic_import(
            filepath=str(abc), as_background_job=False, always_add_cache_reader=True
        )
        self.assertEqual({"FINISHED"}, res)
