...
class UnitTesting(ViewLayerTesting):
    def test_operator_context(self):
        ...
        class SampleOperator(bpy.types.Operator):
            ...
            def execute(self, context):
                ...
                if view_layer.name != self.view_layer:
                    if self.use_verbose:
                        print(
                            'ERROR: Render Layer mismatch: "{0}" != "{1}"'.format(
                ...
                if context.scene_collection.name != scene_collection_name:
                    if self.use_verbose:
                        print(
                            'ERROR: Scene Collection mismatch: "{0}" != "{1}"'.format(
        ...
        # old layer
        self.assertEqual(
            bpy.ops.testing.sample(view_layer="Viewport", use_verbose=True),
            {"FINISHED"},
        )
        ...
        self.assertNotEqual(
            bpy.context.scene_collection.name, layer.collections.active.name
        )
