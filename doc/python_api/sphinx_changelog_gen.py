...
def api_version_previous_in_index(index, version):
    ...
    while True:
        ...
        if version_prev_key in index:
            print(
                "Found previous version %s: %r"
                % (version_prev, index[version_prev_key])
            )
...
def api_dump(args):
    ...
    print(
        "API version %s dumped into %r, and index %r has been updated"
        % (version_key, filepath_out, indexpath)
    )
...
def api_changelog(args):
    ...
    if filepath_in_to is None:
        raise ValueError(
            "Cannot find API dump file for Blender version "
            + str(version)
            + " in index file."
        )
    ...
    if filepath_in_from is None:
        ...
        if version_from is None:
            raise ValueError(
                "No previous version of Blender could be found in the index."
            )
    ...
    with open(
        os.path.join(rootpath, filepath_in_from), "r", encoding="utf-8"
    ) as file_handle:
        _, dict_from = json.load(file_handle)

    with open(
        os.path.join(rootpath, filepath_in_to), "r", encoding="utf-8"
    ) as file_handle:
    ...
    for mod_id, mod_data in dict_to.items():
        ...
        for class_id, class_data in mod_data.items():
            ...
            if props_moved or set_props_new or set_props_old or func_args:
                ...
                api_changes.append(
                    (mod_id, class_id, props_moved, props_new, props_old, func_args)
                )
...
def main(argv=None):
    ...
    "blender --background --factory-startup --python %s -- [options]" % os.path.basename(
        __file__
    )
    ...
    parser_dump = parser_commands.add_parser(
        "dump", help="Dump the current Blender Python API into a JSON file."
    )
