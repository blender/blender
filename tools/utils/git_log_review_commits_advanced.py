...
re_prettify = re.compile(
    r"(.{,20}?)(Fix(?:ing|es)?\s*(?:for)?\s*" + re_bugify_str + r")\s*[-:,]*\s*",
    re.IGNORECASE,
)
...
def release_log_init(
    path, source_dir, blender_rev, start_sha1, end_sha1, rstate, rstate_list
):
    ...
    release_log = {
        "__HEADER__": header,
        "__COUNT__": [0, 0],
        "__RSTATES__": {k: [] for k in rstate_list},
    }

    if os.path.exists(path):
        branch = GitRepo(source_dir).branch.decode().strip()

        sub_cats_to_main_cats = {
            s_cat: m_cat[0] for m_cat in BUGFIX_CATEGORIES for s_cat in m_cat[1]
        }
            ...
            for l in f:
                ...
                if l.startswith("==="):
                    ...
                    if "Fix {{BugReport|" in l:
                        ...
                        _, main_cat_data_unreported = release_log.setdefault(
                            main_cat, ({}, {})
                        )
                    ...
                    if l_rstate.startswith("["):
                        ...
                        if end > 0:
                            ...
                            if rstate in release_log["__RSTATES__"]:
                                release_log["__RSTATES__"][rstate].append(
                                    "* %s" % l_rstate[end + 1:].strip()
                                )
...
def argparse_create():
    ...
    parser.add_argument(
        ...
        required=False,
        help=(
            "Also output more complete info about accepted commits (summary, author...)"
        ),
    )
    parser.add_argument(
        "--accept-pretty",
        ...
        required=False,
        help=(
            "Also output pretty-printed accepted commits (nearly ready for WIKI release notes)"
        ),
    )
    parser.add_argument(
        "--accept-releaselog",
        ...
        required=False,
        help=(
            "Also output accepted commits as a wiki release log page (adds sorting by categories)"
        ),
    )
    parser.add_argument(
        "--blender-rev",
...
def main():
    ...
    args.filter_exclude_sha1_list = {
        sha1.encode() for sha1 in args.filter_exclude_sha1_list
    }
    ...
    if args.accept_releaselog:
        ...
        commits = [
            c for c in GitCommitIter(args.source_dir, args.range_sha1) if match(c)
        ]
                    ...
                    while True:
                        if c1 is None:
                            print(
                                ...
                                % " | ".join(
                                    "[%d] %s" % (i, cat[0])
                                    for i, cat in enumerate(BUGFIX_CATEGORIES)
                                )
                            )
                        else:
