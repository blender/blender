...
if mode is None:
    print(
        "Merge or rebase Blender main (or another base branch) into a branch in 3 steps,"
    )
    ...
    print(
        "                               (assuming that base branch has already been updated"
    )
...
if mode == "rebase":
    ...
    mode_cmd = (
        "merge --no-edit -m \"Merge '" + base_branch + "' into '" + branch + "'\""
    )
...
if code != 0:
    ...
    paths = get_string(
        ("git", "--no-pager", "diff", "--name-only", format_commits[-1])
    ).replace("\n", " ")
    ...
    os.system("git add -u")
    count = int(
        get_string(["git", "rev-list", "--count", "" + format_commits[-1] + "..HEAD"])
    )
