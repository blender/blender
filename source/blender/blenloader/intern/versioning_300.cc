...
extern "C" void reset_clay_mappings(BrushChannelSet *chset, bool strips);
...
void blo_do_versions_300(FileData *fd, Library * /*lib*/, Main *bmain)
{
  ...
  if (!MAIN_VERSION_ATLEAST(bmain, 300, 40)) {
    LISTBASE_FOREACH (Brush *, brush, &bmain->brushes) {
      ...
      Brush temp;
      memcpy(static_cast<void *>(&temp), static_cast<void *>(brush), sizeof(temp));
  ...
  if (MAIN_VERSION_ATLEAST(bmain, 303, 0) && !MAIN_VERSION_ATLEAST(bmain, 303, 1)) {
    Vector<BrushChannelSet *> sets;

    LISTBASE_FOREACH (Brush *, brush, &bmain->brushes) {
      if (brush->channels) {
        sets.append(brush->channels);
      }
    }
    ...
    LISTBASE_FOREACH (Scene *, scene, &bmain->scenes) {
      if (scene->toolsettings && scene->toolsettings->sculpt &&
          scene->toolsettings->sculpt->channels) {
        sets.append(scene->toolsettings->sculpt->channels);
      }
    }
