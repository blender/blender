                                             ...
                                             int *r_tot_info_items)
{
  ...
  const int sdna_nr_preview_image = DNA_struct_find_without_alias(fd->filesdna, "PreviewImage");
                                                 ...
                                                 const char *name)
{
  FileData *fd = (FileData *)bh;
  bool looking = false;
  const int sdna_preview_image = DNA_struct_find_without_alias(fd->filesdna, "PreviewImage");
...
LinkNode *BLO_blendhandle_get_previews(BlendHandle *bh, int ofblocktype, int *r_tot_prev)
{
  ...
  for (bhead = blo_bhead_first(fd); bhead; bhead = blo_bhead_next(fd, bhead)) {
    if (bhead->code == ofblocktype) {
      ...
      if (looking) {
        if (bhead->SDNAnr == DNA_struct_find_without_alias(fd->filesdna, "PreviewImage")) {
