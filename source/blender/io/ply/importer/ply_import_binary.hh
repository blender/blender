...
template<typename T> T swap_bytes(T input)
{
  ...
  if (sizeof(T) == 4) {
    uint32_t newInput = *(uint32_t *)&input;  // Reinterpret this data as uint32 for easy rearranging of bytes
  ...
  if (sizeof(T) == 8) {
    uint64_t newInput = *(uint64_t *)&input;  // Reinterpret this data as uint64 for easy rearranging of bytes
    uint64_t output = 0;
    for (int i = 0; i < 8; i++) {
