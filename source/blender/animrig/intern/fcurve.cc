...
const FCurve *fcurve_find(Span<const FCurve *> fcurves, const FCurveDescriptor &fcurve_descriptor)
{
  for (const FCurve *fcurve : fcurves) {
    ...
    if (fcurve->array_index == fcurve_descriptor.array_index && fcurve->rna_path_ptr &&
        StringRef(fcurve->rna_path_ptr) == fcurve_descriptor.rna_path)
    {
      return fcurve;
    }
  }
...
FCurve *create_fcurve_for_channel(const FCurveDescriptor &fcurve_descriptor)
{
  ...
  fcu->rna_path_ptr = BLI_strdupn(fcurve_descriptor.rna_path.data(),
