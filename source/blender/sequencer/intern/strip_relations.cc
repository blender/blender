...
{
  LISTBASE_FOREACH (Strip *, strip, seqbase) {
    if (strip->type == SEQ_TYPE_SCENE && strip->scene == scene) {
      return strip;
    }

    if (strip->type == SEQ_TYPE_SCENE && (strip->flag & SEQ_SCENE_STRIPS)) {
    ...
    if (strip->type == SEQ_TYPE_META && sequencer_check_scene_recursion(scene, &strip->seqbase)) {
      return strip;
    }
  }
...
bool SEQ_relations_check_scene_recursion(Scene *scene, ReportList *reports)
{
  ...
  if (recursive_seq != nullptr) {
    ...
    LISTBASE_FOREACH (Strip *, strip, &ed->seqbase) {
      if (strip->type != SEQ_TYPE_SCENE && sequencer_seq_generates_image(strip)) {
