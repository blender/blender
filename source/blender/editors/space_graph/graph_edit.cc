    ...
    for (bAnimListElem &ale : anim_data) {
      ...
      if (ale.id && !ale.owner && !fcu->driver) {
        ...
        CombinedKeyingResult result = insert_keyframes(ac->bmain,
                                                       &id_rna_pointer,
                                                       channel_group,
                                                       {{fcu->rna_path_ptr, {}, fcu->array_index}},
                                                       std::nullopt,
                                                       anim_eval_context,
    ...
    const ListBaseT<bAnimListElem> *anim_data, ReportList *reports, int *r_num_groups)
{
  ...
  *r_num_groups = 0;
  ...
  for (bAnimListElem &ale : *anim_data) {
     ...
     */
    if (strstr(fcu->rna_path_ptr, "rotation_euler") == nullptr) {
      continue;
    }
     ...
     * to a new group.
     */
    if ((euf) && (euf->id == ale.id) && STREQ(euf->rna_path, fcu->rna_path_ptr)) {
...
static wmOperatorStatus graph_driver_delete_invalid_exec(bContext *C, wmOperator *op)
{
  ...
  for (bAnimListElem &ale : anim_data) {
    ...
    ok |= ANIM_remove_driver(ale.id, fcu->rna_path_ptr, fcu->array_index);
    if (!ok) {
      break;
    }
    deleted += 1;
