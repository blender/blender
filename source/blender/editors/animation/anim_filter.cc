                                      ...
                                      const eAnimFilter_Flags filter_mode)
{
  ...
  if (GS(owner_id->name) == ID_OB) {
    ...
    if (fcu->rna_path_ptr &&
        BLI_str_quoted_substr(fcu->rna_path_ptr, "pose.bones[", bone_name, sizeof(bone_name)))
    {
    ...
    if (fcu->rna_path_ptr &&
        BLI_str_quoted_substr(fcu->rna_path_ptr, "strips_all[", strip_name, sizeof(strip_name)))
    {
    ...
    if (fcu->rna_path_ptr &&
        BLI_str_quoted_substr(fcu->rna_path_ptr, "nodes[", node_name, sizeof(node_name)))
    {
      /* Get strip name, and check if this strip is selected. */
  ...
  if (filter_mode & ANIMFILTER_LIST_CHANNELS) {
    ...
    if (key->type == KEY_RELATIVE) {
      ...
      for (FCurve *fcurve : fcurves_for_action_slot(action, key->adt->slot_handle)) {
        if (STREQ(fcurve->rna_path_ptr, "eval_time") ||
            BLI_str_endswith(fcurve->rna_path_ptr, ".interpolation"))
        {
