...
static wmOperatorStatus separate_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  for (Base *base_src : bases) {
    src_pointclouds.add(blender::id_cast<PointCloud *>(base_src->object->data));
  }
  ...
  bases.remove_if([&](Base *base) {
    PointCloud *pointcloud = blender::id_cast<PointCloud *>(base->object->data);
  ...
  for (Base *base_src : bases) {
    PointCloud *src = blender::id_cast<PointCloud *>(base_src->object->data);
    ...
    object_dst->data = blender::id_cast<ID *>(dst);
