                         ...
                         int *r_max)
{
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (i == 0) {
      ...
      min_id = std::min(min_id, *ids);
      max_id = std::max(max_id, *ids);
    }
...
static void handle_missing_id_layers(Mesh *src,
                                     ...
                                     int totpoly)
{
  const CustomData *src_datas[4] = {
      &src->vert_data, &src->edge_data, &src->corner_data, &src->face_data};
  CustomData *dst_datas[4] = {vdata, edata, ldata, pdata};
  int srctots[4] = {src->verts_num, src->edges_num, src->corners_num, src->faces_num};
