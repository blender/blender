...
{
  if (seq::SEQ_retiming_is_last_key(strip, key)) {
    return seq::SEQ_retiming_key_timeline_frame_get(scene, strip, key) + 1;
  }
  return seq::SEQ_retiming_key_timeline_frame_get(scene, strip, key);
}
...
{
  return UI_view2d_view_to_region_x(v2d, seq::SEQ_time_left_handle_frame_get(scene, strip));
}

static float strip_end_screenspace_get(const Scene *scene, const View2D *v2d, const Strip *strip)
{
  return UI_view2d_view_to_region_x(v2d, seq::SEQ_time_right_handle_frame_get(scene, strip));
}
...
int left_fake_key_frame_get(const bContext *C, const Strip *strip)
{
  ...
  const int sound_offset = seq::SEQ_time_get_rounded_sound_offset(strip, scene_fps);
  const int content_start = seq::SEQ_time_start_frame_get(strip) + sound_offset;
  return max_ii(content_start, seq::SEQ_time_left_handle_frame_get(scene, strip));
}
...
int right_fake_key_frame_get(const bContext *C, const Strip *strip)
{
  ...
  const float scene_fps = float(scene->r.frs_sec) / float(scene->r.frs_sec_base);
  const int sound_offset = seq::SEQ_time_get_rounded_sound_offset(strip, scene_fps);
  const int content_end = seq::SEQ_time_content_end_frame_get(scene, strip) - 1 + sound_offset;
  return min_ii(content_end, seq::SEQ_time_right_handle_frame_get(scene, strip));
}
                                            ...
                                            int &r_frame)
{
  ...
  if (seq::SEQ_time_right_handle_frame_get(scene, strip) >=
      seq::SEQ_time_content_end_frame_get(scene, strip))
  {
    right_x += 1;
  }
...
void realize_fake_keys(const Scene *scene, Strip *strip)
{
  ...
  seq::SEQ_retiming_add_key(scene, strip, seq::SEQ_time_left_handle_frame_get(scene, strip));
  seq::SEQ_retiming_add_key(scene, strip, seq::SEQ_time_right_handle_frame_get(scene, strip));
}
...
SeqRetimingKey *try_to_realize_fake_keys(const bContext *C, Strip *strip, const int mval[2])
{
  ...
  if (retiming_fake_key_frame_clicked(C, strip, mval, key_frame)) {
    ...
    key = seq::SEQ_retiming_key_get_by_timeline_frame(scene, strip, key_frame);
  }
  return key;
}
                                                     ...
                                                     const int mval[2])
{
  ...
  for (SeqRetimingKey &key : seq::SEQ_retiming_keys_get(strip)) {
    ...
    if (key_x_get(scene, strip, &key) == seq::SEQ_time_left_handle_frame_get(scene, strip) ||
        key_x_get(scene, strip, &key) == seq::SEQ_time_right_handle_frame_get(scene, strip))
    {
                              ...
                              const StripDrawContext &strip_ctx)
{
  ...
  if (!seq::SEQ_retiming_is_allowed(strip_ctx.strip)) {
    return false;
  }
                            ...
                            const KeyframeShaderBindings &sh_bindings)
{
  ...
  if (seq::SEQ_retiming_key_is_freeze_frame(key)) {
  ...
  const float alpha = seq::SEQ_retiming_data_is_editable(strip) ? 1.0f : 0.3f;

  draw_keyframe_shape(key_position,
                      bottom,
                      size,
                      is_selected && seq::SEQ_retiming_data_is_editable(strip),
                      key_type,
                      KEYFRAME_SHAPE_BOTH,
                      alpha,
                      &sh_bindings,
                                        ...
                                        const StripDrawContext &strip_ctx)
{
  if (!can_draw_retiming(timeline_ctx, strip_ctx) ||
      seq::SEQ_retiming_keys_count(strip_ctx.strip) == 0)
  {
    return;
  }
  ...
  for (const SeqRetimingKey &key : seq::SEQ_retiming_keys_get(strip)) {
...
static SeqRetimingKey fake_retiming_key_init(const Scene *scene, const Strip *strip, int key_x)
{
  ...
  fake_key.strip_frame_index = (key_x - seq::SEQ_time_start_frame_get(strip) - sound_offset) *
                           ...
                           const KeyframeShaderBindings &sh_bindings)
{
  ...
  if (!seq::SEQ_retiming_is_active(strip) && !seq::SEQ_retiming_data_is_editable(strip)) {
    return false;
  }

  const int left_key_frame = left_fake_key_frame_get(timeline_ctx->C, strip);
  if (seq::SEQ_retiming_key_get_by_timeline_frame(scene, strip, left_key_frame) == nullptr) {
  ...
  if (seq::SEQ_retiming_key_get_by_timeline_frame(scene, strip, right_key_frame) == nullptr) {
    /* `key_x_get()` compensates 1 frame offset of last key, however this can not
     * be conveyed via `fake_key` alone. Therefore the same offset must be emulated. */
    if (strip_ctx.right_handle >= seq::SEQ_time_content_end_frame_get(scene, strip)) {
      right_key_frame += 1;
    }
                                  ...
                                  blender::Span<StripDrawContext> strips)
{
  ...
  for (const StripDrawContext &strip_ctx : strips) {
    ...
    for (const SeqRetimingKey &key : seq::SEQ_retiming_keys_get(strip_ctx.strip)) {
                            ...
                            const size_t label_str_maxncpy)
{
  ...
  if (seq::SEQ_retiming_key_is_transition_start(key)) {
    const float prev_speed = seq::SEQ_retiming_key_speed_get(strip, key);
    const float next_speed = seq::SEQ_retiming_key_speed_get(strip, next_key + 1);
  ...
  const float speed = seq::SEQ_retiming_key_speed_get(strip, next_key);
