...
static void set_prop_dist(TransInfo *t, const bool with_dist)
{
  ...
  blender::KDTree_3d *td_tree = blender::kdtree_3d_new(td_table_len);
  ...
  FOREACH_TRANS_DATA_CONTAINER (t, tc) {
    tc->foreach_index_selected([&](const int i) {
      ...
      blender::kdtree_3d_insert(td_tree, td_table_index, vec);
      td_table[td_table_index++] = td;
    });
  }
  BLI_assert(td_table_index == td_table_len);

  blender::kdtree_3d_balance(td_tree);
  ...
  FOREACH_TRANS_DATA_CONTAINER (t, tc) {
    tc->foreach_index([&](const int i) {
      ...
      const int td_index = blender::kdtree_3d_find_nearest(td_tree, vec, &nearest);
  ...
  blender::kdtree_3d_free(td_tree);
  MEM_freeN(td_table);
}
