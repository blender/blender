...
void drawSnapping(TransInfo *t)
{
  ...
  if (t->spacetype == SPACE_SEQ) {
    ...
    ui::UI_GetThemeColor3ubv(TH_TRANSFORM, col);
    col[3] = 128;

    ui::UI_GetThemeColor3ubv(TH_SELECT, selectedCol);
    selectedCol[3] = 128;

    ui::UI_GetThemeColor3ubv(TH_ACTIVE, activeCol);
    activeCol[3] = 192;
  }
  ...
  if (t->spacetype == SPACE_VIEW3D) {
    ...
    if (!BLI_listbase_is_empty(&t->tsnap.points)) {
      ...
      float size = 2.0f * blender::ui::UI_GetThemeValuef(TH_VERTEX_SIZE);
      float view_inv[4][4];
    ...
    blender::ui::UI_view2d_view_to_region_fl(&t->region->v2d, UNPACK2(snap_point), &x, &y);
    float radius = 2.5f * blender::ui::UI_GetThemeValuef(TH_VERTEX_SIZE) * U.pixelsize;
