...
static void SeqTransInfo(TransInfo *t, Strip *strip, int *r_count, int *r_flag)
{
  ...
  if (t->mode == TFM_TIME_EXTEND) {
    ...
    int left = SEQ_time_left_handle_frame_get(scene, strip);
    int right = SEQ_time_right_handle_frame_get(scene, strip);

    if ((strip->flag & SELECT) == 0 || SEQ_transform_is_locked(channels, strip)) {
      *r_count = 0;
      *r_flag = 0;
    }
    else {
                                 ...
                                 int sel_flag)
{
  ...
  switch (sel_flag) {
    case SELECT:
      ...
      start_left = SEQ_time_left_handle_frame_get(scene, strip);
    ...
    case SEQ_LEFTSEL:
      ...
      break;
    case SEQ_RIGHTSEL:
      td2d->loc[0] = SEQ_time_right_handle_frame_get(scene, strip);
      break;
  }
...
static void seq_transform_cancel(TransInfo *t, Span<Strip *> transformed_strips)
{
  ...
  for (Strip *strip : transformed_strips) {
    ...
    if (SEQ_transform_test_overlap(t->scene, seqbase, strip)) {
      SEQ_transform_seqbase_shuffle(seqbase, strip, t->scene);
    }
  }
}
                                               ...
                                               VectorSet<Strip *> &time_dependent_strips)
{
  ...
  VectorSet selected_strips = SEQ_query_selected_strips(seqbase);
...
static void flushTransSeq(TransInfo *t)
{
    ...
    switch (tdsq->sel_flag) {
      case SELECT: {
        if (SEQ_transform_sequence_can_be_translated(strip)) {
          ...
          SEQ_transform_translate_sequence(scene, strip, offset);
          if (abs(offset) > abs(max_offset)) {
            max_offset = offset;
          }
        }
        strip->machine = round_fl_to_int(td->loc[1] + edge_pan_offset[1]);
        CLAMP(strip->machine, 1, SEQ_MAX_CHANNELS);
        break;
      }
      case SEQ_LEFTSEL: { /* No vertical transform. */
        ...
        if (both_handles_selected && new_frame > SEQ_time_left_handle_frame_get(scene, strip)) {
          a++, td++, td2d++;
          int new_right_frame = round_fl_to_int(td->loc[0] + edge_pan_offset[0]);
          SEQ_time_right_handle_frame_set(scene, strip, new_right_frame);
        }
        int old_startdisp = SEQ_time_left_handle_frame_get(scene, strip);
        SEQ_time_left_handle_frame_set(t->scene, strip, new_frame);

        if (abs(SEQ_time_left_handle_frame_get(scene, strip) - old_startdisp) > abs(max_offset)) {
          max_offset = SEQ_time_left_handle_frame_get(scene, strip) - old_startdisp;
        }
        break;
      }
      case SEQ_RIGHTSEL: { /* No vertical transform. */
        int old_enddisp = SEQ_time_right_handle_frame_get(scene, strip);
        SEQ_time_right_handle_frame_set(t->scene, strip, new_frame);

        if (abs(SEQ_time_right_handle_frame_get(scene, strip) - old_enddisp) > abs(max_offset)) {
          max_offset = SEQ_time_right_handle_frame_get(scene, strip) - old_enddisp;
        }
        break;
      }
    }
  ...
  for (Strip *strip : ts->time_dependent_strips) {
    SEQ_offset_animdata(t->scene, strip, max_offset);
  }
