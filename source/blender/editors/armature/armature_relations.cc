    ...
    Main *bmain, ID *id, FCurve *fcu, Object *srcArm, Object *tarArm, GHash *names_map)
{
  ...
  if ((id == src_id) && strstr(fcu->rna_path_ptr, "pose.bones[")) {
    GHASH_ITER (gh_iter, names_map) {
      ...
      if (!STREQ(old_name, new_name) && strstr(fcu->rna_path_ptr, old_name)) {
        fcu->rna_path_ptr = BKE_animsys_fix_rna_path_rename(
            id, fcu->rna_path_ptr, "pose.bones", old_name, new_name);
