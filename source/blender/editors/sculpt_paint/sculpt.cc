...
float SCULPT_get_float_intern(const SculptSession *ss,
                              ...
                              const Brush *br)
{
  BrushMappingData *mapdata = ss->cache ? &ss->cache->input_mapping : nullptr;
...
void SCULPT_vertex_limit_surface_get(SculptSession *ss, PBVHVertRef vertex, float r_co[3])
{
  if (BKE_pbvh_type(ss->pbvh) != PBVH_GRIDS) {
    if (ss->attrs.limit_surface) {
      float *f = (float *)SCULPT_vertex_attr_get(vertex, ss->attrs.limit_surface);
      copy_v3_v3(r_co, f);
    }
    else {
...
void sculpt_vertex_faceset_update_bmesh(SculptSession *ss, PBVHVertRef vert)
{
  ...
  MSculptVert *mv = (MSculptVert *)BM_ELEM_CD_GET_VOID_P(v, ss->cd_sculpt_vert);
...
static bool neighbor_cache_begin(const SculptSession *ss)
{
  ...
  if (atomic_cas_ptr((void **)&ss->cache->ncache, nullptr, ncache) != nullptr) {
                                             ...
                                             const bool include_duplicates)
{
  ...
  if (include_duplicates && !ncache->duplicates) {
    ...
    if (atomic_cas_ptr(&ncache->duplicates, nullptr, duplicates) != nullptr) {
  ...
  int thread_nr = BLI_task_parallel_thread_id(nullptr);
  ...
  if (!cache[i]) {
    ...
    // if (atomic_cas_ptr(&cache[i], nullptr, item) != item) {
    // another thread got here first
    //}

    atomic_cas_ptr((void **)&cache[i], nullptr, item);
  }
...
{
  eSculptCorner flag = *(eSculptCorner *)SCULPT_vertex_attr_get(vertex, ss->attrs.boundary_flags);
...
{
  eSculptBoundary flag = *(eSculptBoundary *)SCULPT_vertex_attr_get(vertex,
...
static bool sculpt_brush_needs_rake_rotation(const SculptSession *ss, const Brush *brush)
{
  return SCULPT_TOOL_HAS_RAKE(SCULPT_get_tool(ss, brush)) &&
         (SCULPT_get_float(ss, rake_factor, nullptr, brush) != 0.0f);
}
                                         ...
                                         eBrushFalloffShape falloff_mode)
{
  ...
  if (ss->cache && ss->cache->channels_final) {
    tip_roundness = SCULPT_get_float(ss, tip_roundness, nullptr, nullptr);
    tip_scale_x = SCULPT_get_float(ss, tip_scale_x, nullptr, nullptr);
  }
                                            ...
                                            float tip_scale_x)
{
  ...
  SculptBrushTestFn sculpt_brush_test_sq_fn = nullptr;
...
// avg = 0.0f;
      ...
      avg = BKE_brush_sample_tex_3d(scene, br, &br->mtex, point_3d, rgba, thread_id, ss->tex_pool);
    }
    else {
...
{
  float factor = BRUSHSET_GET_FLOAT(ss->cache->channels_final, smooth_strength_factor, nullptr);
  float projection = BRUSHSET_GET_FLOAT(
      ss->cache->channels_final, smooth_strength_projection, nullptr);
...
{
  SculptSession *ss = ob->sculpt;
  BrushRunCommandData *data = (BrushRunCommandData *)userdata;
  BrushCommand *cmd = data->cmd;

  float radius;

  if (BRUSHSET_GET_INT(cmd->params_mapped, radius_unit, nullptr)) {
    radius = BRUSHSET_GET_FLOAT(cmd->params_mapped, unprojected_radius, nullptr);
  }
  else {
    radius = BRUSHSET_GET_FLOAT(cmd->params_mapped, radius, nullptr);
  ...
  ss->cache->use_plane_trim = BRUSHSET_GET_INT(cmd->params_mapped, use_plane_trim, nullptr);
  float plane_trim = BRUSHSET_GET_FLOAT(cmd->params_mapped, plane_trim, nullptr);
  ...
  ups->alpha = BRUSHSET_GET_FLOAT(cmd->params_final, strength, nullptr);

  if (cmd->tool == SCULPT_TOOL_SMOOTH) {
    ss->cache->bstrength = BRUSHSET_GET_FLOAT(cmd->params_mapped, strength, nullptr);
    if (ss->cache->invert) {
  ...
  brush2->alpha = BRUSHSET_GET_FLOAT(cmd->params_final, strength, nullptr);

  if (!BRUSHSET_GET_INT(cmd->params_mapped, use_ctrl_invert, nullptr)) {
  ...
  switch (brush2->sculpt_tool) {
    ...
    case SCULPT_TOOL_SMOOTH:
      if (brush2->smooth_deform_type == BRUSH_SMOOTH_DEFORM_LAPLACIAN) {
        SCULPT_do_smooth_brush(sd,
                               ...
                               nodes,
                               totnode,
                               BRUSHSET_GET_FLOAT(cmd->params_mapped, projection, nullptr),
    ...
    case SCULPT_TOOL_DYNTOPO:
      sculpt_topology_update(sd, ob, brush, ups, nullptr, paint_mode_settings);
      break;
                                   ...
                                   PaintModeSettings *paint_mode_settings)
{
  ...
  for (int i = 0; i < list->totcommand; i++) {
    ...
    memcpy((void *)&brush2, brush, sizeof(brush2));
    ...
    if (BRUSHSET_GET_INT(cmd->params_final, radius_unit, nullptr)) {
                                               ...
                                               const TaskParallelTLS *__restrict /*tls*/)
{
  ...
  MutableSpan<float3> verts = me->vert_positions_for_write();
                                         ...
                                         void *userdata)
{
  ...
  for (int i = 0; i <= symm; i++) {
    ...
    const ePaintSymmetryFlags symmf = ePaintSymmetryFlags(i);
    cache->mirror_symmetry_pass = symmf;
    cache->radial_symmetry_pass = 0;

    SCULPT_cache_calc_brushdata_symm(cache, symmf, 0, 0);

    do_tiled(sd, ob, brush, ups, paint_mode_settings, action, userdata);

    do_radial_symmetry(
        sd, ob, brush, ups, paint_mode_settings, action, symmf, 'X', feather, userdata);
    do_radial_symmetry(
        sd, ob, brush, ups, paint_mode_settings, action, symmf, 'Y', feather, userdata);
    do_radial_symmetry(
        sd, ob, brush, ups, paint_mode_settings, action, symmf, 'Z', feather, userdata);
  }
}
...
static BrushChannelSet *sculpt_init_tool_override_channels(Sculpt *sd, SculptSession *ss, int tool)
{
  ...
  Brush dummy2{};
  dummy2.sculpt_tool = tool;
  dummy2.channels = chset;
    ...
    bContext *C, Sculpt *sd, SculptSession *ss, wmOperator *op, const float mval[2])
{
  ...
  float plane_trim = BRUSHSET_GET_FINAL_FLOAT(sd->channels, channels, plane_trim, nullptr);
...
{
  if (SCULPT_get_tool(ss, brush) == SCULPT_TOOL_CLOTH) {
    return SCULPT_get_int(ss, cloth_deform_type, nullptr, brush) != BRUSH_CLOTH_DEFORM_GRAB;
  }
...
static void sculpt_update_brush_delta(UnifiedPaintSettings *ups, Object *ob, Brush *brush)
{
  ...
  bad = bad && SCULPT_get_float(ss, tip_roundness, nullptr, brush) == 1.0f;
  bad = bad && SCULPT_get_float(ss, tip_scale_x, nullptr, brush) == 1.0f;

  bad = bad && !sculpt_brush_use_topology_rake(ss, brush);
  bad = bad && !SCULPT_get_bool(ss, use_autofset, nullptr, brush);
  ...
  if (!SCULPT_stroke_is_first_brush_step_of_symmetry_pass(ss->cache)) {
    if (sculpt_needs_delta_from_anchored_origin(ss, brush)) {
      ...
      if (brush->flag & (BRUSH_ANCHORED | BRUSH_DRAG_DOT)) {
        ...
        if (SCULPT_get_int(ss, use_smoothed_rake, nullptr, brush)) {
          float tmp1[3];
          float tmp2[3];
  ...
  if (cache->has_cubic) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (!SCULPT_stroke_get_location(C, cache->world_cubic[i], mouse_cubic[i], false)) {
        ...
        mul_v3_m4v3(loc, ob->object_to_world, cache->true_location);
...
void SCULPT_flush_update_done(const bContext *C, Object *ob, SculptUpdateType update_flags)
{
  ...
  if (BKE_pbvh_type(ss->pbvh) == PBVH_BMESH) {
    ...
    if (update_flags & SCULPT_UPDATE_COLOR) {
      ...
      BKE_pbvh_search_gather(ss->pbvh, all_nodes_callback, nullptr, &nodes, &totnode);
                                          ...
                                          BrushMappingData *input_data)
{
  ...
  if (BRUSHSET_GET_BOOL(chset, dyntopo_disabled, nullptr)) {
    r_settings->flag |= DYNTOPO_DISABLED;
  }

  r_settings->flag = BRUSHSET_GET_INT(chset, dyntopo_mode, nullptr);
  r_settings->mode = BRUSHSET_GET_INT(chset, dyntopo_detail_mode, nullptr);
...
// bad debug global
extern "C" bool pbvh_show_orig_co;
...

{
  ...
  sd->smooth_strength_factor = BRUSHSET_GET_FLOAT(
      ss->cache->channels_final, smooth_strength_factor, nullptr);
  ...
  sculpt_cache_dyntopo_settings(ss->cache->channels_final,
                                ...
                                ss->cache ? &ss->cache->input_mapping : nullptr);
  ...
  ss->cache->boundary_symmetry = (ePaintSymmetryFlags)boundsym;
  ...
  if (detail_mode == DYNTOPO_DETAIL_CONSTANT || detail_mode == DYNTOPO_DETAIL_MANUAL) {
    float object_space_constant_detail = 1.0f /
                                         (constant_detail * mat4_to_scale(ob->object_to_world));
...
extern "C" const EnumPropertyItem rna_enum_brush_sculpt_tool_items[];
static EnumPropertyItem *stroke_tool_items = nullptr;
...
static void SCULPT_fake_neighbor_init(SculptSession *ss, const float max_dist)
{
  ...
  ss->fake_neighbors.fake_neighbor_index = static_cast<PBVHVertRef *>(
      MEM_malloc_arrayN(totvert, sizeof(PBVHVertRef), "fake neighbor"));
...
{
  float object_space_constant_detail = 1.0f /
                                       (cd->detail_size * mat4_to_scale(cd->active_object->object_to_world));
                                                    ...
                                                    DyntopoDetailSizeEditCustomData *cd)
{
  ...
  if (num_neighbors > 0) {
    ...
    const float detail_size = 0.7f / (avg_edge_len * mat4_to_scale(cd->active_object->object_to_world));
