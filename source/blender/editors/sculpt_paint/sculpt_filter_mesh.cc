...
void cache_free(SculptSession *ss, Object * /*ob*/)
{
  ...
  BKE_pbvh_vertex_iter_begin (ss->pbvh, node, vd, PBVH_ITER_UNIQUE) {
    ...
    switch (filter_type) {
      ...
      case MESH_FILTER_RELAX: {
        smooth::relax_vertex(ss, &vd, clamp_f(fade, 0.0f, 1.0f), SCULPT_BOUNDARY_MESH, val);
      ...
      case MESH_FILTER_RELAX_FACE_SETS: {
        ...
        smooth::relax_vertex(ss, &vd, clamp_f(fade, 0.0f, 1.0f), boundtype, val);
        sub_v3_v3v3(disp, val, vd.co);
        break;
      }
      case MESH_FILTER_SURFACE_SMOOTH: {
        smooth::surface_smooth_laplacian_step(
            ss, disp, vd.co, vd.vertex, orig_data.co, 1.0f, true);
        break;
      }
...
static void mesh_filter_enhance_details_init_directions(SculptSession *ss)
{
  ...
  for (int i = 0; i < totvert; i++) {
    ...
    float avg[3];
    smooth::neighbor_coords_average(ss, avg, vertex, 0.0f, filter_cache->hard_corner_pin, true);
