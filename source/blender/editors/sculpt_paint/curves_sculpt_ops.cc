...
{
  if (BKE_brush_use_size_pressure(nullptr, &brush, false)) {
...
{
  return BKE_brush_size_get(&scene, &brush, false) * brush_radius_factor(brush, stroke_extension);
}

float brush_strength_factor(const Brush &brush, const StrokeExtension &stroke_extension)
{
  if (BKE_brush_use_alpha_pressure(nullptr, &brush, false)) {
    return stroke_extension.pressure;
  }
  return 1.0f;
}
...
static int calculate_points_per_side(bContext *C, MinDistanceEditData &op_data)
{
  ...
  const float brush_radius = BKE_brush_size_get(scene, op_data.brush, false);
...
static void min_distance_edit_draw(bContext *C, int /*x*/, int /*y*/, void *customdata)
{
  ...
  float brush_radius_re = BKE_brush_size_get(scene, op_data.brush, false);
