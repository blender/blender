                                         ...
                                         const PBVHFaceRef f)
{
  ...
  if (expand_cache->snap_enabled_face_sets) {
    const int faceset = expand_cache->original_face_sets[f_i];
    enabled = expand_cache->snap_enabled_face_sets->contains(faceset);
  }
  else {
...
static void sculpt_expand_vertex_to_faces_falloff_bmesh(BMesh *bm, expand::Cache *expand_cache)
{
  BMIter iter;
  BMFace *f;
                                                       ...
                                                       const BitSpan enabled_verts)
{
  ...
  for (int i = 0; i < totvert; i++) {
    ...
    PBVHVertRef vertex = BKE_pbvh_index_to_vertex(ss->pbvh, i);
    flood_fill::add_and_skip_initial(ss, &flood, vertex);
  }
...
static void sculpt_expand_original_state_store(Object *ob, Cache *expand_cache)
{
  ...
  expand_cache->initial_face_sets = face_set::duplicate_face_sets(*ob);
