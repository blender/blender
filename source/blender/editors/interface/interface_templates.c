...
void uiTemplateStatusInfo(uiLayout *layout, bContext *C)
{
  ...
  UI_fontstyle_set(&style->widgetlabel);
  int width = (int)BLF_width(
      style->widgetlabel.uifont_id, status_info_txt, strlen(status_info_txt));
  width = max_ii(width, (int)(10 * UI_DPI_FAC));
...
void uiTemplateCacheFileProcedural(uiLayout *layout, const bContext *C, PointerRNA *fileptr)
{
  ...
  const bool engine_supports_procedural = RE_engine_supports_alembic_procedural(engine_type,
                                                                                scene);
  CacheFile *cache_file = (CacheFile *)fileptr->data;
  CacheFile *cache_file_eval = (CacheFile *)DEG_get_evaluated_id(CTX_data_depsgraph_pointer(C),
                                                                 &cache_file->id);
