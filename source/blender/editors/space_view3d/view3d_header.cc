...
void uiTemplateEditModeSelection(blender::ui::Layout *layout, bContext *C)
{
  ...
  PointerRNA op_ptr = row.op(
      ...
      (em->selectmode & SCE_SELECT_VERTEX) ? blender::ui::UI_ITEM_O_DEPRESS : UI_ITEM_NONE);
  RNA_enum_set(&op_ptr, "type", SCE_SELECT_VERTEX);
  op_ptr = row.op(ot,
                  "",
                  ICON_EDGESEL,
                  blender::wm::OpCallContext::InvokeDefault,
                  (em->selectmode & SCE_SELECT_EDGE) ? blender::ui::UI_ITEM_O_DEPRESS :
                                                       UI_ITEM_NONE);
  ...
  op_ptr = row.op(ot,
                  ...
                  ICON_FACESEL,
                  blender::wm::OpCallContext::InvokeDefault,
                  (em->selectmode & SCE_SELECT_FACE) ? blender::ui::UI_ITEM_O_DEPRESS :
                                                       UI_ITEM_NONE);
...
static void uiTemplatePaintModeSelection(blender::ui::Layout *layout, bContext *C)
{
  ...
  if (!ELEM(ob->mode, OB_MODE_SCULPT, OB_MODE_PARTICLE_EDIT)) {
    ...
    if (ob->mode & OB_MODE_TEXTURE_PAINT) {
      layout->prop(&meshptr, "use_paint_mask", blender::ui::UI_ITEM_R_ICON_ONLY, "", ICON_NONE);
    }
    else {
      blender::ui::Layout &row = layout->row(true);
      row.prop(&meshptr, "use_paint_mask", blender::ui::UI_ITEM_R_ICON_ONLY, "", ICON_NONE);
      row.prop(&meshptr, "use_paint_mask_vertex", blender::ui::UI_ITEM_R_ICON_ONLY, "", ICON_NONE);

      /* Show the bone selection mode icon only if there is a pose mode armature */
      Object *ob_armature = BKE_object_pose_armature_get(ob);
      if (ob_armature) {
        row.prop(
            &meshptr, "use_paint_bone_selection", blender::ui::UI_ITEM_R_ICON_ONLY, "", ICON_NONE);
      }
    }
  }
}
