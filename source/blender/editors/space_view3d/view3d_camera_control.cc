                                                     ...
                                                     RegionView3D *rv3d)
{
  ...
  if ((rv3d->persp == RV3D_CAMOB) && (rv3d->is_persp == false)) {
    (blender::id_cast<Camera *>(v3d->camera->data))->type = CAM_PERSP;
...
void ED_view3d_cameracontrol_release(View3DCameraControl *vctrl, const bool restore)
{
  ...
  if (restore) {
    ...
    DEG_id_tag_update(blender::id_cast<ID *>(view3d_cameracontrol_object(vctrl)),
                      ID_RECALC_TRANSFORM);
  ...
  if (vctrl->is_ortho_cam) {
    (blender::id_cast<Camera *>(v3d->camera->data))->type = CAM_ORTHO;
  }
