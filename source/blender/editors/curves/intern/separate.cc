...
static wmOperatorStatus separate_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  for (Base *base_src : bases) {
    src_curves.add(blender::id_cast<Curves *>(base_src->object->data));
  }
  ...
  bases.remove_if([&](Base *base) {
    Curves *curves = blender::id_cast<Curves *>(base->object->data);
  ...
  for (Base *base_src : bases) {
    Curves *src = blender::id_cast<Curves *>(base_src->object->data);
