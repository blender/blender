    ...
    switch (vbo.type) {
      case CD_MLOOPUV: {
        const int cd_uv = CustomData_get_offset_named(
            &args->bm->ldata, CD_MLOOPUV, vbo.name.c_str());

        foreach_bmesh([&](BMLoop *l) {
          MLoopUV *uv = static_cast<MLoopUV *> BM_ELEM_CD_GET_VOID_P(l, cd_uv);

          *static_cast<float2 *>(GPU_vertbuf_raw_step(&access)) = uv->uv;
        });
