...
struct ViewCulling {
  [[uniform(DRW_VIEW_CULLING_UBO_SLOT)]] const ViewCullingData (&drw_view_culling_buf)[64];

  ViewCullingData get(uint view_id) const
  {
    return drw_view_culling_buf[view_id];
  }

  bool intersect_view(Pyramid pyramid, uint view_id) const
  {
    bool intersects = true;

    /* WORKAROUND: There is a GLSL compiler bug on legacy AMD GPU drivers that returns an incorrect
     * computation if `drw_view_culling_buf[]` is called in both of the loops below (see #143336).
     */
    ViewCullingData culling = drw_view_culling_buf[view_id];
    ...
    for (int p = 0; p < 5; ++p) {
      ...
      for (int v = 0; v < 8; ++v) {
        float test = dot(i_pyramid.planes[p],
                         float4(culling.frustum_corners.corners[v].xyz, 1.0f));
        if (test > 0.0f) {
          is_any_vertex_on_positive_side = true;
          break;
        }
  ...
  bool intersect_view(Box box, uint view_id) const
  {
    bool intersects = true;
  ...
  bool intersect_view(IsectBox i_box, uint view_id) const
  {
    bool intersects = true;
  ...
  bool intersect_view(Sphere sphere, uint view_id) const
  {
    bool intersects = true;
