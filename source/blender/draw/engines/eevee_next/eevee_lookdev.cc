...
LookdevWorld::LookdevWorld()
{
  ...
  bNode *rotate = bke::nodeAddStaticNode(nullptr, ntree, SH_NODE_VECTOR_ROTATE);
  rotate->custom1 = NODE_VECTOR_ROTATE_TYPE_AXIS_Z;
  bNodeSocket *rotate_vector_in = bke::nodeFindSocket(rotate, SOCK_IN, "Vector");
  angle_socket_ = static_cast<bNodeSocketValueFloat *>(
      static_cast<void *>(bke::nodeFindSocket(rotate, SOCK_IN, "Angle")->default_value));
  bNodeSocket *rotate_out = bke::nodeFindSocket(rotate, SOCK_OUT, "Vector");

  bNode *environment = bke::nodeAddStaticNode(nullptr, ntree, SH_NODE_TEX_ENVIRONMENT);
  environment_node_ = environment;
  NodeTexImage *environment_storage = static_cast<NodeTexImage *>(environment->storage);
  bNodeSocket *environment_vector_in = bke::nodeFindSocket(environment, SOCK_IN, "Vector");
  bNodeSocket *environment_out = bke::nodeFindSocket(environment, SOCK_OUT, "Color");
  ...
  bNodeSocket *background_color_in = bke::nodeFindSocket(background, SOCK_IN, "Color");
  intensity_socket_ = static_cast<bNodeSocketValueFloat *>(
      static_cast<void *>(bke::nodeFindSocket(background, SOCK_IN, "Strength")->default_value));
  ...
  bke::nodeAddLink(ntree, coordinate, coordinate_out, rotate, rotate_vector_in);
  bke::nodeAddLink(ntree, rotate, rotate_out, environment, environment_vector_in);
  bke::nodeAddLink(ntree, environment, environment_out, background, background_color_in);
