...
float light_shape_radius(LightData light)
{
  ...
  return light.local().shape_radius;
}
...
float light_point_light(LightData light, const bool is_directional, LightVector lv)
{
  ...
  float r_sqr = square(light.local().shape_radius);
...
{
  if (is_sphere_light(light.type) && lv.dist < light.local().shape_radius) {
  ...
  if (light.type == LIGHT_RECT) {
    ...
    if (is_sphere_light(light.type)) {
      ...
      size = float2(light_sphere_disk_radius(light.local().shape_radius, lv.dist));
    }
    else if (is_oriented_disk_light(light.type)) {
      /* View direction-aligned disk. */
      size = float2(light.local().shape_radius);
    }
