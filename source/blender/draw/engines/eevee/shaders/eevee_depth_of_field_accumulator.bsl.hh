...
#define coc_radius_error float(2.0f)
  ...
  float dof_intersection_weight(float coc,
                                         ...
                                         DofGatherData &accum_data)
  {
    ...
    for (int i = 0; i < 2; i++) {
       ...
       * If not, we accumulate to the ring bucket. This is to have more consistent sample
       * occlusion.
       */
                                           ...
                                           DofGatherData &accum_data)
  {
    ...
    if (!do_fast_gather) {
      ...
      dof_gather_accumulate_sample_ring(
          center_data, 1, false, do_fast_gather, foreground, accum_data);
    }
  }
                                                        ...
                                                        int density_change)
  {
    int sample_count_per_density_change = dof_gather_total_sample_count(ring_count, ring_density) -
                                          dof_gather_total_sample_count(
                                              ring_count - gather_density_change_ring,
                                              ring_density);
  ...
  void dof_gather_init(float2 frag_coord,
                       float base_radius,
                       float2 noise,
                       float2 &center_co,
                       float &lod,
  ...
  void dof_gather_accumulator(float2 frag_coord,
                              sampler2D color_tx,
                              ...
                              float2 &out_occlusion)
  {
    ...
    float lod, isect_mul;
    float2 center_co;
    dof_gather_init(frag_coord + 0.5f, base_radius, noise, center_co, lod, isect_mul);
  ...
  void dof_slight_focus_gather(float2 frag_coord,
                               ...
                               float &out_center_coc)
  {
    ...
    dof_gather_accumulate_resolve(
        total_sample_count, bg_accum, bg_col, bg_weight, unused_occlusion);
    dof_gather_accumulate_resolve(
        total_sample_count, fg_accum, fg_col, fg_weight, unused_occlusion);
