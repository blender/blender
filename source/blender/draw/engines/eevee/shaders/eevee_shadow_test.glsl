...
void main()
{
  TEST(eevee_shadow, DirectionalMemberSet)
  {
    ...
    EXPECT_EQ(light_sun_data_get(light).clipmap_origin, vec2(3.0f, 4.0f));
  ...
  TEST(eevee_shadow, DirectionalClipmapLevel)
  {
    ...
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * 0.0f)),
              light_sun_data_get(light).clipmap_lod_min);
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * 0.49f)), 1);
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * 0.5f)), 1);
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * 0.51f)), 1);
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * 0.99f)), 2);
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * 1.0f)), 2);
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * 1.01f)), 2);
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * 12.5f)), 6);
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * 12.51f)), 6);
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * 15.9999f)), 6);
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * 16.0f)), 6);
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * 16.00001f)), 6);
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * 5000.0f)),
  ...
  TEST(eevee_shadow, DirectionalCascadeLevel)
  {
    ...
    float fac = float(SHADOW_TILEMAP_RES - 1) / float(SHADOW_TILEMAP_RES);
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * half_size * 0.0f, 0.0f, 0.0f)), 2);
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * half_size * 0.5f, 0.0f, 0.0f)), 2);
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * half_size * 1.0f, 0.0f, 0.0f)), 3);
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * half_size * 1.5f, 0.0f, 0.0f)), 3);
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * half_size * 2.0f, 0.0f, 0.0f)), 4);
    EXPECT_EQ(shadow_directional_level(light, vec3(fac * 5000.0f)),
              light_sun_data_get(light).clipmap_lod_max);
    /* Produces NaN / Inf, Undefined behavior. */
    // EXPECT_EQ(shadow_directional_level(light, vec3(FLT_MAX)),
  ...
  TEST(eevee_shadow, DirectionalCascadeCoordinates)
  {
    ...
    vec2 farthest_tilemap_center = vec2(lod_half_size * float(level_range_size - 1), 0.0f);
    set_clipmap_base_offset(
        light, floatBitsToInt(vec2(lod_half_size / float(level_range_size - 1), 0.0f)));

    /* Test UVs and tile mapping. */

    lP = vec3(1e-8f, 1e-8f, 0.0f);
    coords = shadow_directional_coordinates(light, lP);
    EXPECT_EQ(coords.tilemap_index, 1);
    EXPECT_EQ(coords.tilemap_tile, uvec2(SHADOW_TILEMAP_RES / 2));
    // EXPECT_NEAR(coords.uv, vec2(SHADOW_TILEMAP_RES / 2), 1e-3f);

    lP = vec3(lod_half_size * narrowing - 1e-5f, 1e-8f, 0.0f);
    coords = shadow_directional_coordinates(light, lP);
    EXPECT_EQ(coords.tilemap_index, 1);
    ...
    lP = vec3(lod_half_size + 1e-5f, 1e-5f, 0.0f);
    coords = shadow_directional_coordinates(light, lP);
    EXPECT_EQ(coords.tilemap_index, 2);
    EXPECT_EQ(coords.tilemap_tile, uvec2(SHADOW_TILEMAP_RES - 1, SHADOW_TILEMAP_RES / 2));
