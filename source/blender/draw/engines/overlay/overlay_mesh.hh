...
class MeshUVs : Overlay {
 ...
 public:
  void begin_sync(Resources &res, const State &state) final
  {
    ...
    if (select_vert_) {
      const float dot_size = ui::UI_GetThemeValuef(TH_VERTEX_SIZE) * UI_SCALE_FAC;
      float4 theme_color;
      ui::UI_GetThemeColor4fv(TH_VERTEX, theme_color);
    ...
    if (select_face_dots_) {
      const float dot_size = ui::UI_GetThemeValuef(TH_FACEDOT_SIZE) * UI_SCALE_FAC;
  ...
  void end_sync(Resources &res, const State &state) final
  {
    ...
    if (show_tiled_image_border_) {
       ...
       * required. */
      ui::UI_GetThemeColorShade4ubv(TH_BACK, 60, text_color);
      ui::UI_GetThemeColorShade4fv(TH_BACK, 60, theme_color);
      ui::UI_GetThemeColor4fv(TH_FACE_SELECT, selected_color);
      srgb_to_linearrgb_v4(theme_color, theme_color);
