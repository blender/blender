...
void main()
{
  ...
  final_inner_color = flag_test(bone_flag, COL_HEAD) ? data_buf[gpu_InstanceIndex].head_color :
                                                       data_buf[gpu_InstanceIndex].tail_color;
  final_inner_color = flag_test(bone_flag, COL_BONE) ? data_buf[gpu_InstanceIndex].bone_color :
                                                       final_inner_color;
  final_wire_color = (data_buf[gpu_InstanceIndex].wire_color.a > 0.0f) ?
                         data_buf[gpu_InstanceIndex].wire_color :
                         final_inner_color;
  color_fac = flag_test(bone_flag, COL_WIRE) ? 0.0f :
                                               (flag_test(bone_flag, COL_BONE) ? 1.0f : 2.0f);

  float4 boneStart_4d = float4(data_buf[gpu_InstanceIndex].bone_start.xyz, 1.0f);
  float4 boneEnd_4d = float4(data_buf[gpu_InstanceIndex].bone_end.xyz, 1.0f);
  ...
  vpos *= (data_buf[gpu_InstanceIndex].wire_color.a > 0.0f) ? 1.0f : 0.5f;
