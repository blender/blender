  ...
  void begin_sync(Resources &res, const State &state) final
  {
    ...
    res.select_bind(ps_);
    {
      ...
      /* Selection Boxes. */
      {
        ...
        ui::UI_GetThemeColor4fv(TH_WIDGET_TEXT_SELECTION, color);
      ...
      /* Highlight text within selection boxes. */
      {
        ...
        sub.shader_set(res.shaders->uniform_color.get());
        ui::UI_GetThemeColor4fv(TH_WIDGET_TEXT_HIGHLIGHT, color);
        srgb_to_linearrgb_v4(color, color);
        sub.push_constant("ucolor", color);
      ...
      /* Cursor (text caret). */
      {
        ...
        sub.shader_set(res.shaders->uniform_color.get());
        sub.state_set(default_state, state.clipping_plane_count);
        ui::UI_GetThemeColor4fv(TH_WIDGET_TEXT_CURSOR, color);
        srgb_to_linearrgb_v4(color, color);
        sub.push_constant("ucolor", color);
        cursor_ps_ = &sub;
      }
