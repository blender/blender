...
{
  layout.prop(ptr, "from_instancer", ui::UI_ITEM_R_SPLIT_EMPTY_NAME, std::nullopt, ICON_NONE);
  ...
  if (!RNA_boolean_get(ptr, "from_instancer")) {
    ...
    layout.prop(ptr, "uv_map", ui::UI_ITEM_R_SPLIT_EMPTY_NAME, std::nullopt, ICON_GROUP_UVS);
  }
}
