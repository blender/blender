...
struct CompositorNodesSrnaContext {
  StructRNA *properties_struct;
  StructPathFunc path_func;
};
...
static const Strip *find_effect_strip_from_system_property(const PointerRNA *ptr)
{
  if (const auto strip = RNA_struct_search_closest_ancestor_by_type(ptr, RNA_Strip)) {
    return static_cast<const Strip *>(strip->data);
  }
    ...
    const PointerRNA *ptr)
{
  ...
  return fmt::format("sequence_editor.strips_all[\"{}\"].modifiers[\"{}\"].properties.inputs.{}",
                     strip_name_esc,
                     modifier_name_esc,
                     identifier);
}
    ...
    const PointerRNA *ptr)
{
  StructRNA *srna = ptr->type;
  const char *identifier = RNA_struct_identifier(srna);
  const Strip *strip = find_effect_strip_from_system_property(ptr);
  BLI_assert(strip);
  std::string strip_name_esc = BLI_str_escape(strip->name + 2);
  return fmt::format(
      "sequence_editor.strips_all[\"{}\"].properties.inputs.{}", strip_name_esc, identifier);
}
