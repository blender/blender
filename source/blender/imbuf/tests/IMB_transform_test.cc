...
static ImBuf *transform_2x_smaller(eIMBInterpolationFilterMode filter)
{
  ...
  ImBuf *dst = IMB_allocImBuf(3, 1, IB_byte_data);
...
{
  ImBuf *src = create_6x2_test_image();
  ImBuf *dst = IMB_allocImBuf(9, 7, IB_byte_data);
...
TEST_F(ImBufTransformTest, nearest_very_large_scale)
{
  ...
  ImBuf *src = IMB_allocImBuf(511, 1, IB_byte_data);
  ...
  ImBuf *res = IMB_allocImBuf(3841, 1, IB_byte_data);
