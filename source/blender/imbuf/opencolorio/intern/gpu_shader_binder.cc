 ...
 */
static void process_source(std::string &source, blender::gpu::shader::ShaderCreateInfo &info)
{
  source_comment_out_uniforms(source);
  source = GPU_shader_preprocess_source(source, info);
    ...
    const Span<std::array<StringRefNull, 2>> additional_defines)
{
  ...
  process_source(info.fragment_source_generated, info);
