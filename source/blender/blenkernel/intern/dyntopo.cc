                                              ...
                                              float *r_w)
{
  ...
  if ((eq_ctx->mode & PBVH_Subdivide) && len1 > max) {
    if (r_w) {
      *r_w = len1;
    }
  ...
  if ((eq_ctx->mode & PBVH_Collapse) && len2 < min) {
    if (r_w) {
      *r_w = len2;
    }
  ...
  for (int i = 0; i < totface; i++) {
    ...
    for (j = 0; j < n; j++) {
      ...
      if (newf) {
        ...
        float w = 0.0f;
        if (test_near_brush(rl->e, co, &w)) {
          long_edge_queue_edge_add_recursive(eq_ctx, rl, w, eq_ctx->limit_len_max, 0);
        }
#endif
