...
static void material_copy_data(Main *bmain,
                               ...
                               const int flag)
{
  Material *material_dst = blender::id_cast<Material *>(id_dst);
  const Material *material_src = blender::id_cast<const Material *>(id_src);
...
static void material_free_data(ID *id)
{
  ...
  BKE_icon_id_delete(blender::id_cast<ID *>(material));
}
...
Material ***BKE_id_material_array_p(ID *id)
{
  ...
  switch (GS(id->name)) {
    case ID_ME:
      return &((blender::id_cast<Mesh *>(id))->mat);
    case ID_CU_LEGACY:
      return &((blender::id_cast<Curve *>(id))->mat);
    case ID_MB:
      return &((blender::id_cast<MetaBall *>(id))->mat);
    case ID_GD_LEGACY:
      return &((blender::id_cast<bGPdata *>(id))->mat);
    case ID_CV:
      return &((blender::id_cast<Curves *>(id))->mat);
    case ID_PT:
      return &((blender::id_cast<PointCloud *>(id))->mat);
    case ID_VO:
      return &((blender::id_cast<Volume *>(id))->mat);
    case ID_GP:
      return &((blender::id_cast<GreasePencil *>(id))->material_array);
    default:
      break;
  }
  return nullptr;
...
short *BKE_id_material_len_p(ID *id)
{
  ...
  switch (GS(id->name)) {
    case ID_ME:
      return &((blender::id_cast<Mesh *>(id))->totcol);
    case ID_CU_LEGACY:
      return &((blender::id_cast<Curve *>(id))->totcol);
    case ID_MB:
      return &((blender::id_cast<MetaBall *>(id))->totcol);
    case ID_GD_LEGACY:
      return &((blender::id_cast<bGPdata *>(id))->totcol);
    case ID_CV:
      return &((blender::id_cast<Curves *>(id))->totcol);
    case ID_PT:
      return &((blender::id_cast<PointCloud *>(id))->totcol);
    case ID_VO:
      return &((blender::id_cast<Volume *>(id))->totcol);
    case ID_GP:
      return &((blender::id_cast<GreasePencil *>(id))->material_array_num);
    default:
      break;
  }
  return nullptr;
...
static void material_data_index_remove_id(ID *id, short index)
{
  ...
  switch (GS(id->name)) {
    case ID_ME:
      BKE_mesh_material_index_remove(blender::id_cast<Mesh *>(id), index);
      break;
    case ID_CU_LEGACY:
      BKE_curve_material_index_remove(blender::id_cast<Curve *>(id), index);
      break;
    case ID_GP:
...
bool BKE_object_material_slot_used(Object *object, short actcol)
{
  ...
  switch (GS(ob_data->name)) {
    case ID_ME:
      return BKE_mesh_material_index_used(blender::id_cast<Mesh *>(ob_data), actcol - 1);
    case ID_CU_LEGACY:
      return BKE_curve_material_index_used(blender::id_cast<Curve *>(ob_data), actcol - 1);
    case ID_MB:
...
static void material_data_index_clear_id(ID *id)
{
  ...
  switch (GS(id->name)) {
    case ID_ME:
      BKE_mesh_material_index_clear(blender::id_cast<Mesh *>(id));
      break;
    case ID_CU_LEGACY:
      BKE_curve_material_index_clear(blender::id_cast<Curve *>(id));
      break;
    case ID_MB:
    case ID_CV:
    case ID_PT:
  ...
  *materials_len_p_dst = *materials_len_p_src;
  if (*materials_len_p_src != 0) {
    ...
    for (int a = 0; a < *materials_len_p_src; a++) {
      id_us_plus(blender::id_cast<ID *>((*matar_dst)[a]));
    }
...
void BKE_id_material_resize(Main *bmain, ID *id, short totcol, bool do_id_user)
{
  ...
  if (do_id_user && totcol < (*totcolp)) {
    ...
    for (i = totcol; i < (*totcolp); i++) {
      id_us_min(blender::id_cast<ID *>((*matar)[i]));
    }
  }
...
Material *BKE_id_material_pop(Main *bmain, ID *id, int index_i)
{
  ...
  if (matar) {
    ...
    if (index >= 0 && index < (*totcol)) {
      ret = (*matar)[index];
      id_us_min(blender::id_cast<ID *>(ret));
...
void BKE_id_material_clear(Main *bmain, ID *id)
{
  ...
  if (matar) {
    ...
    while ((*totcol)--) {
      id_us_min(blender::id_cast<ID *>((*matar)[*totcol]));
    }
    *totcol = 0;
    if (*matar) {
      MEM_freeN(*matar);
...
void BKE_object_material_resize(Main *bmain, Object *ob, const short totcol, bool do_id_user)
{
  ...
  if (do_id_user && totcol < ob->totcol) {
    for (int i = totcol; i < ob->totcol; i++) {
      id_us_min(blender::id_cast<ID *>(ob->mat[i]));
    }
  }
...
void BKE_object_material_remap(Object *ob, const uint *remap)
{
  ...
  if (ob->type == OB_MESH) {
    BKE_mesh_material_remap(blender::id_cast<Mesh *>(ob->data), remap, ob->totcol);
  }
  else if (ELEM(ob->type, OB_CURVES_LEGACY, OB_SURF, OB_FONT)) {
    BKE_curve_material_remap(blender::id_cast<Curve *>(ob->data), remap, ob->totcol);
  }
  else if (ob->type == OB_GREASE_PENCIL) {
    BKE_grease_pencil_material_remap(
        blender::id_cast<GreasePencil *>(ob->data), remap, ob->totcol);
  }
  else {
  ...
  for (bNode *node : nodetree->all_nodes()) {
    ...
    if (do_color_attributes && node->typeinfo->type_legacy == SH_NODE_ATTRIBUTE) {
      ...
      if (!ntree_foreach_texnode_recursive(
              blender::id_cast<bNodeTree *>(node->id), callback, userdata, slot_filter))
      {
        return false;
      }
    }
...
static bool fill_texpaint_slots_cb(bNode *node, void *userdata)
{
  ...
  switch (node->type_legacy) {
    case SH_NODE_TEX_IMAGE: {
      ...
      slot->ima = blender::id_cast<Image *>(node->id);
    ...
    case SH_NODE_ATTRIBUTE: {
      ...
      if (storage->type == SHD_ATTRIBUTE_GEOMETRY) {
        const Mesh *mesh = blender::id_cast<const Mesh *>(fill_data->ob->data);
...
static bool texpaint_slot_node_find_cb(bNode *node, void *userdata)
{
  ...
  if (find_data->slot->ima && node->type_legacy == SH_NODE_TEX_IMAGE) {
    Image *node_ima = blender::id_cast<Image *>(node->id);
