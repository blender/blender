...
template<typename T> static T *copy_vector_to_c_array(const char *tag, Vector<T> &vector)
{
  if (vector.size() == 0) {
    return nullptr;
  }

  void *ret = MEM_malloc_arrayN(sizeof(T), vector.size(), tag);
  memcpy(ret, static_cast<void *>(vector.data()), sizeof(T) * vector.size());
...
#define NORMAL_VERT_BAD(v) \
  ...
  data->border_verts = (BMVert **)MEM_malloc_arrayN(
      bordervs.size(), sizeof(void *), "data->border_verts");
  memcpy((void *)data->border_verts, (void *)bordervs.data(), sizeof(void *) * bordervs.size());

  data->tot_border_verts = bordervs.size();
...
void pbvh_bmesh_normals_update(PBVH *pbvh, PBVHNode **nodes, int totnode)
{
  ...
  UpdateNormalsTaskData *datas = (UpdateNormalsTaskData *)MEM_calloc_arrayN(
      totnode, sizeof(*datas), "bmesh normal update");
    ...
    if (e->l) {
      ...
      if (totuv) {
        ...
        do {
          ...
          for (int i = 0; i < totuv; i++) {
            float *luv = (float *)BM_ELEM_CD_GET_VOID_P(l, cd_uvs[i]);
...
static void coalese_pbvh(PBVH *pbvh, BMesh *bm)
{
  ...
  int *fmap = (int *)MEM_calloc_arrayN(bm->totface, sizeof(int), "pbvh face map");
  int totleaf = 0;

  Vector<FastNodeBuildInfo> nodes;

  const int qsize = leafsize;
  BMFace **queue = (BMFace **)MEM_calloc_arrayN(qsize, sizeof(*queue), "pbvh queue");
  ...
  BM_ITER_MESH (f, &iter, bm, BM_FACES_OF_MESH) {
    ...
    FastNodeBuildInfo *node = &nodes[nodes.size() - 1];
  ...
  for (int step = 0; step < 55; step++) {
    ...
    for (int i = starti; i < endi; i++) {
      FastNodeBuildInfo *n1 = &nodes[i], *n2 = nullptr;
                                          ...
                                          bool is_snake_hook)
{
  ...
  for (int j : IndexRange(nodes.size())) {
...
static bool pbvh_bmesh_split_tris(PBVH *pbvh, PBVHNode *node)
{
  ...
  Vector<PBVHVertRef> verts;
  Vector<PBVHTri> tris;
  Vector<intptr_t> loops;
    ...
    do {
      ...
      BM_ITER_ELEM (l2, &iter, l, BM_LOOPS_OF_VERT) {
        ...
        for (int i = 0; i < totlayer; i++) {
          float *uv1 = (float *)BM_ELEM_CD_GET_VOID_P(l, cd_uv + cd_size * i);
          float *uv2 = (float *)BM_ELEM_CD_GET_VOID_P(l2, cd_uv + cd_size * i);
  ...
  if (node->tribuf) {
    ...
    node->tribuf = (PBVHTriBuf *)MEM_callocN(sizeof(*node->tribuf), "node->tribuf");
  }

  node->tribuf->verts = copy_vector_to_c_array<PBVHVertRef>("node->tribuf->verts", verts);
  node->tribuf->loops = copy_vector_to_c_array<intptr_t>("node->tribuf->loops", loops);
  node->tribuf->tris = copy_vector_to_c_array<PBVHTri>("node->tribuf->tris", tris);

  node->tribuf->tottri = tris.size();
  node->tribuf->totvert = verts.size();
  node->tribuf->totloop = loops.size();
...
BLI_INLINE PBVHTri *pbvh_tribuf_add_tri(PBVHTriBuf *tribuf)
{
  ...
  if (tribuf->tottri >= tribuf->tris_size) {
    ...
    if (!tribuf->tris) {
      tribuf->tris = (PBVHTri *)MEM_mallocN(sizeof(*tribuf->tris) * newsize, "tribuf tris");
    }
    else {
      tribuf->tris = (PBVHTri *)MEM_reallocN_id(
          tribuf->tris, sizeof(*tribuf->tris) * newsize, "tribuf tris");
    }
...
BLI_INLINE void pbvh_tribuf_add_vert(PBVHTriBuf *tribuf, PBVHVertRef vertex, BMLoop *l)
{
  ...
  if (tribuf->totvert >= tribuf->verts_size) {
    ...
    if (!tribuf->verts) {
      tribuf->verts = (PBVHVertRef *)MEM_mallocN(sizeof(*tribuf->verts) * newsize, "tribuf verts");
      tribuf->loops = (intptr_t *)MEM_mallocN(sizeof(*tribuf->loops) * newsize, "tribuf loops");
    }
    else {
...
BLI_INLINE void pbvh_tribuf_add_edge(PBVHTriBuf *tribuf, int v1, int v2)
{
  ...
  if (tribuf->totedge >= tribuf->edges_size) {
    ...
    if (!tribuf->edges) {
      tribuf->edges = (int *)MEM_mallocN(sizeof(*tribuf->edges) * 2ULL * newsize, "tribuf edges");
    }
    else {
...
bool BKE_pbvh_bmesh_check_tris(PBVH *pbvh, PBVHNode *node)
{
  ...
  Vector<BMLoop *> loops;
  Vector<blender::uint3> loops_idx;
  ...
  TGSET_ITER (f, node->bm_faces) {
    ...
    BM_face_calc_tessellation(
        f, true, loops.data(), reinterpret_cast<uint(*)[3]>(loops_idx.data()));
...
#  endif
  ...
  node->tri_buffers = copy_vector_to_c_array<PBVHTriBuf>("node->tri_buffers", tribufs);
...
void BKE_pbvh_bmesh_update_valence(int cd_sculpt_vert, PBVHVertRef vertex)
{
  ...
  MSculptVert *mv = (MSculptVert *)BM_ELEM_CD_GET_VOID_P(v, cd_sculpt_vert);
...
static void pbvh_bmesh_compact_tree(PBVH *bvh)
{
  ...
  int *map = (int *)MEM_callocN(sizeof(int) * bvh->totnode, "bmesh map temp");
...
static void pbvh_bmesh_balance_tree(PBVH *pbvh)
{
  ...
  float *overlaps = (float *)MEM_calloc_arrayN(pbvh->totnode, sizeof(float), "overlaps");
  PBVHNode **parentmap = (PBVHNode **)MEM_calloc_arrayN(
      pbvh->totnode, sizeof(*parentmap), "parentmap");
  int *depthmap = (int *)MEM_calloc_arrayN(pbvh->totnode, sizeof(*depthmap), "depthmap");
...
static void pbvh_bmesh_join_nodes(PBVH *bvh)
{
  ...
  BKE_pbvh_bmesh_correct_tree(bvh, bvh->nodes, nullptr);
  ...
  for (int i = 0; i < bvh->totnode; i++) {
    for (int j = 0; j < bvh->totnode; j++) {
      ...
      if (bvh->nodes[i].draw_batches == bvh->nodes[j].draw_batches) {
        ...
        bvh->nodes[j].draw_batches = nullptr;
      }
    }
  }
...
static void scan_edge_split(BMesh *bm, BMEdge **edges, int totedge)
{
  ...
  for (int i = 0; i < totedge; i++) {
    ...
    BMVert *v2 = (BMVert *)BLI_mempool_alloc(bm->vpool);
    memset(v2, 0, sizeof(*v2));
    v2->head.data = BLI_mempool_alloc(bm->vdata.pool);

    newverts.append(v2);

    BMEdge *e2 = (BMEdge *)BLI_mempool_alloc(bm->epool);
    newedges.append(e2);
    ...
    do {
      faces.append(l->f);
      BMFace *f2 = (BMFace *)BLI_mempool_alloc(bm->fpool);
...
BMesh *BKE_pbvh_reorder_bmesh(PBVH *pbvh)
{
  ...
  vidx = (uint *)MEM_malloc_arrayN(pbvh->header.bm->totvert, sizeof(*vidx), "vorder");
  eidx = (uint *)MEM_malloc_arrayN(pbvh->header.bm->totedge, sizeof(*eidx), "eorder");
  lidx = (uint *)MEM_malloc_arrayN(pbvh->header.bm->totloop, sizeof(*lidx), "lorder");
  fidx = (uint *)MEM_malloc_arrayN(pbvh->header.bm->totface, sizeof(*fidx), "forder");
...
#  define VISIT_TAG BM_ELEM_TAG
  ...
  for (int i = 0; i < pbvh->totnode; i++) {
    for (int j = 0; j < nodedata[i].totface; j++) {
      ...
      BMFace *f2 = BM_face_create(bm2, fvs, fes, totloop, nullptr, BM_CREATE_NOP);
...
BMesh *BKE_pbvh_reorder_bmesh1(PBVH *pbvh)
{
  ...
  int **save_other_vs = (int **)MEM_calloc_arrayN(pbvh->totnode, sizeof(int *), __func__);
  int **save_unique_vs = (int **)MEM_calloc_arrayN(pbvh->totnode, sizeof(int *), __func__);
  int **save_fs = (int **)MEM_calloc_arrayN(pbvh->totnode, sizeof(int *), __func__);

  SortElem *verts = (SortElem *)MEM_malloc_arrayN(bm->totvert, sizeof(SortElem), __func__);
  SortElem *edges = (SortElem *)MEM_malloc_arrayN(bm->totedge, sizeof(SortElem), __func__);
  SortElem *faces = (SortElem *)MEM_malloc_arrayN(bm->totface, sizeof(SortElem), __func__);
  ...
  for (i = 0; i < pbvh->totnode; i++) {
    Vector<int> other_vs, unique_vs, fs;
    ...
    save_unique_vs[i] = copy_vector_to_c_array<int>(__func__, unique_vs);
    save_other_vs[i] = copy_vector_to_c_array<int>(__func__, other_vs);
    save_fs[i] = copy_vector_to_c_array<int>(__func__, fs);
  }
  ...
  uint *vs = (uint *)MEM_malloc_arrayN(bm->totvert, sizeof(int), __func__);
  uint *es = (uint *)MEM_malloc_arrayN(bm->totedge, sizeof(int), __func__);
  uint *fs = (uint *)MEM_malloc_arrayN(bm->totface, sizeof(int), __func__);
  ...
  // create new mappings
  BMVert **mapvs = (BMVert **)MEM_malloc_arrayN(bm->totvert, sizeof(BMVert *), __func__);
  BMEdge **mapes = (BMEdge **)MEM_malloc_arrayN(bm->totedge, sizeof(BMEdge *), __func__);
  BMFace **mapfs = (BMFace **)MEM_malloc_arrayN(bm->totface, sizeof(BMFace *), __func__);
...
{
  MeshTest *m = (MeshTest *)MEM_callocN(sizeof(MeshTest), "MeshTest");
...
void pbvh_bmesh_cache_test(CacheParams *params, BMesh **r_bm, PBVH **r_pbvh_out)
{
  ...
  BMeshCreateParams params2 = {0};

  params2.id_elem_mask = BM_VERT | BM_EDGE | BM_FACE;
  params2.id_map = true;
  params2.create_unique_ids = true;
  params2.temporary_ids = false;
  params2.no_reuse_ids = false;

  BMesh *bm = BM_mesh_create(&templ, &params2);
  ...
  BMVert **grid = (BMVert **)MEM_malloc_arrayN(steps * steps, sizeof(*grid), "bmvert grid");
  ...
  for (uint i = 0; i < 4; i++) {
    rands[i] = (uint *)MEM_malloc_arrayN(tots[i], sizeof(uint), "rands[i]");
...
static void hash_test()
{
  ...
  int *data = (int *)MEM_callocN(sizeof(*data) * count, "test data");
...
extern "C" void pbvh_bmesh_do_cache_test()
{
  for (int i = 0; i < 15; i++) {
    printf("\n\n====== %d of %d =====\n", i + 1, 15);
    hash_test();
