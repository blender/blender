...
static ShrinkwrapBoundaryData *shrinkwrap_build_boundary_data(struct Mesh *mesh)
{
  ...
  if (num_boundary_edges == 0) {
    MEM_freeN((void *)edge_is_boundary);
    MEM_freeN((void *)edge_mode);
    return nullptr;
  }

  /* Allocate the data object. */
  ShrinkwrapBoundaryData *data = (ShrinkwrapBoundaryData *)MEM_callocN(
      sizeof(ShrinkwrapBoundaryData), "ShrinkwrapBoundaryData");
  ...
  int *vert_boundary_id = (int *)MEM_calloc_arrayN(
  ...
  ShrinkwrapBoundaryVertData *boundary_verts = (ShrinkwrapBoundaryVertData *)MEM_calloc_arrayN(
      num_boundary_verts, sizeof(*boundary_verts), "ShrinkwrapBoundaryData::boundary_verts");

  signed char *vert_status = (signed char *)MEM_calloc_arrayN(
      num_boundary_verts, sizeof(char), __func__);
...
{
  ShrinkwrapCalcCBData *data = (ShrinkwrapCalcCBData *)userdata;

  ShrinkwrapCalcData *calc = data->calc;
  BVHTreeFromMesh *treeData = &data->tree->treeData;
  BVHTreeNearest *nearest = (BVHTreeNearest *)tls->userdata_chunk;
...
static void shrinkwrap_ray_callback(void *userdata,
                                    ...
                                    BVHTreeRayHit *hit)
{
  const ShrinkWrapRayData *sdata = (ShrinkWrapRayData *)userdata;
                                   ...
                                   BVHTreeRayHit *hit)
{
  ...
  if (options & MOD_SHRINKWRAP_CULL_TARGET_MASK) {
    if (options & MOD_SHRINKWRAP_CULL_TARGET_FRONTFACE) {
      mask |= 1;
    }
    if (options & MOD_SHRINKWRAP_CULL_TARGET_BACKFACE) {
      mask |= 2;
    }
  }
  else {
  ...
  ShrinkWrapRayData sdata = {&tree->treeData, mask};

  BLI_bvhtree_ray_cast(
      tree->bvh, co, no, ray_radius, &hit_tmp, shrinkwrap_ray_callback, &sdata);
                                                    ...
                                                    const TaskParallelTLS *__restrict tls)
{
  ...
  BVHTreeRayHit *hit = (BVHTreeRayHit *)tls->userdata_chunk;
  ...
  if (hit->index != -1) {
    ...
    if (calc->ropt) {
      ...
      //copy_v3_v3(rv->targetco, hit->co);
      copy_v3_v3(rv->targetno, hit->no);
    }

    interp_v3_v3v3(co, co, hit->co, weight * calc->shrinkRatio);
                                                        ...
                                                        const TaskParallelTLS *__restrict tls)
{
  ...
  if (nearest->index != -1) {
    ...
    if (calc->ropt) {
      ReliefVert *rv = calc->ropt->verts + i;

      copy_v3_v3(rv->targetco, tmp_co);
      copy_v3_v3(rv->targetno, nearest->no);
    }

    interp_v3_v3v3(co, co, tmp_co, weight * calc->shrinkRatio); /* linear interpolation */
  }
}
                               ...
                               int numVerts)
{
  ...
  if (mesh != nullptr && smd->shrinkType == MOD_SHRINKWRAP_PROJECT) {
    ...
    if (smd->subsurfLevels) {
      ...
      ss_mesh = subsurf_make_derived_from_derived(
          ...
          scene,
          nullptr,
          (SubsurfFlags)((ob->mode & OB_MODE_EDIT) ? SUBSURF_IN_EDIT_MODE : 0));

      if (ss_mesh) {
        calc.vert = (MVert *)ss_mesh->getVertDataArray(ss_mesh, CD_MVERT);
        if (calc.vert) {
        ...
        MEdge *medge = (MEdge *)ss_mesh->getEdgeDataArray(ss_mesh, CD_MEDGE);

        if (smd->optimizeNormals) {
          calc.ropt = new ReliefOptimizer(vertexCos,
                                          calc.vert,
                                          ss_mesh->getNumVerts(ss_mesh),
                                          medge,
                                          medge ? ss_mesh->getNumEdges(ss_mesh) : 0);
  ...
  if (!calc.ropt && smd->optimizeNormals) {
    calc.ropt = new ReliefOptimizer(
        vertexCos, mesh->mvert, mesh->totvert, mesh->medge, mesh->totedge);
    calc.ropt->compress_ratio = smd->rayShrinkRatio;
  }
                                                ...
                                                Object *ob_target)
{
  ...
  Mesh *src_me = (Mesh *)ob_source->data;
