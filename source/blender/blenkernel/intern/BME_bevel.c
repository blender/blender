...
BME_Poly *BME_split_faceb(BME_Mesh *bm, BME_Poly *f, BME_Vert *v1, BME_Vert *v2, BME_Loop **nl, BME_Edge *example) {
	BME_Poly *nf;
...
BME_Loop *BME_bevel_edge(BME_Mesh *bm, BME_Loop *l, float value, int options, float *up_vec, BME_TransData_Head *td) {
	...
	if (kv) {
		l = l->next;
		if (kl->v == kv) {
			BME_split_faceb(bm,kl->f,kl->prev->v,kl->next->v,&nl,kl->prev->e);
			BME_JFKE(bm,((BME_Loop*)kl->prev->radial.next->data)->f,kl->f,kl->prev->e);
			BME_JEKV(bm,kl->e,kv);
		}
		else {
			BME_split_faceb(bm,kl->f,kl->next->next->v,kl->v,&nl,kl->next->e);
	...
	if ((v1->tflag1 & BME_BEVEL_NONMAN)==0 || (v2->tflag1 & BME_BEVEL_NONMAN)==0) {
		BME_split_faceb(bm,f,v2,v1,&l,e);
...
BME_Loop *BME_bevel_vert(BME_Mesh *bm, BME_Loop *l, float value, int options, float *up_vec, BME_TransData_Head *td) {
	...
	/* "cut off" this corner */
	f = BME_split_faceb(bm,l->f,v2,v1,NULL,l->e);
	...
	for (v = bm->verts.first; v; /* we may kill v, so increment in-loop */) {
		...
		if ((v->tflag1 & BME_BEVEL_NONMAN) && (v->tflag1 & BME_BEVEL_BEVEL) && (v->tflag1 & BME_BEVEL_ORIG)) {
			...
			for (i=0,e=v->edge; i < len; i++,e=BME_disk_nextedge(e,v)) {
				...
				if (l->f->len > 3) {
					BME_split_faceb(bm,l->f,l->next->v,l->prev->v,&l,l->e); /* clip this corner off */
					if (len > 2) {
						l->e->tflag1 |= BME_BEVEL_BEVEL;
					}
				}
				if (l2->f->len > 3) {
					BME_split_faceb(bm,l2->f,l2->next->v,l2->prev->v,&l,l2->e); /* clip this corner off */
					if (len > 2) {
						l->e->tflag1 |= BME_BEVEL_BEVEL;
					}
				}
...
BME_Mesh *BME_tesselate(BME_Mesh *bm) {
	...
	for (f=bm->polys.first; f; f=f->next) {
		...
		while (l->f->len > 4) {
			...
			/* make a quad */
			BME_split_faceb(bm,l->f,l->v,nextloop->v,NULL,l->e);
			l = nextloop;
		}
	}
	return bm;
