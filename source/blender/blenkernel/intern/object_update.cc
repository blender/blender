                                     ...
                                     const bool is_from_set)
{
  ...
  BLI_assert(view_layer->runtime->object_bases_array != nullptr);
  BLI_assert(base_index >= 0);
  BLI_assert(base_index <
             MEM_allocN_len(view_layer->runtime->object_bases_array) / sizeof(Base *));
  Base *base = view_layer->runtime->object_bases_array[base_index];
