...
static void panel_draw(const bContext * /*C*/, Panel *panel)
{
  ...
  col->prop(ptr, "radius", ui::UI_ITEM_R_SLIDER, IFACE_("Line Radius"), ICON_NONE);
  col->prop(ptr, "opacity", ui::UI_ITEM_R_SLIDER, std::nullopt, ICON_NONE);
...
static void edge_types_panel_draw(const bContext * /*C*/, Panel *panel)
{
  ...
  if (use_cache && !is_first) {
    ...
    sub->prop(ptr,
              "crease_threshold",
              ui::UI_ITEM_R_SLIDER | blender::ui::UI_ITEM_R_FORCE_BLANK_DECORATE,
              std::nullopt,
              ICON_NONE);
  }
...
static void material_mask_panel_draw(const bContext * /*C*/, Panel *panel)
{
  ...
  for (int i = 0; i < 8; i++) {
    sub->prop(ptr, prop, i, 0, ui::UI_ITEM_R_TOGGLE, " ", ICON_NONE);
    if (i == 3) {
      sub = &col.row(true);
    }
  }
...
static void chaining_panel_draw(const bContext * /*C*/, Panel *panel)
{
  ...
  layout.prop(ptr, "smooth_tolerance", ui::UI_ITEM_R_SLIDER, std::nullopt, ICON_NONE);
  layout.prop(ptr, "split_angle", ui::UI_ITEM_R_SLIDER, std::nullopt, ICON_NONE);
}
...
static void vgroup_panel_draw(const bContext * /*C*/, Panel *panel)
{
  ...
  row.prop(ptr, "invert_source_vertex_group", ui::UI_ITEM_R_TOGGLE, "", ICON_ARROW_LEFTRIGHT);
...
static void bake_panel_draw(const bContext * /*C*/, Panel *panel)
{
  ...
  if (is_baked) {
    ...
    col.prop(
        ptr, "is_baked", ui::UI_ITEM_R_TOGGLE, IFACE_("Continue Without Clearing"), ICON_NONE);
  }
...
static void composition_panel_draw(const bContext * /*C*/, Panel *panel)
{
  ...
  col.prop(ptr, "stroke_depth_offset", ui::UI_ITEM_R_SLIDER, IFACE_("Depth Offset"), ICON_NONE);
  col.prop(ptr,
