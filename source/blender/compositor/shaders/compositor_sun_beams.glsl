...
void main()
{
  ...
  float unbounded_steps = max(1.0f, distance(vec2(texel), source * input_size));
  ...
  vec2 coordinates = (vec2(texel) + vec2(0.5f)) / input_size;
  ...
  for (int i = 0; i <= steps; i++) {
     ...
     * boundaries, so break. */
    if (any(lessThan(position, vec2(0.0f))) || any(greaterThan(position, vec2(1.0f)))) {
      break;
    }
