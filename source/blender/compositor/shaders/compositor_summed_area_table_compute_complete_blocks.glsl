...
void main()
{
  ...
  if (gl_LocalInvocationID.x == 0) {
    ...
    vec4 x_accumulated_color = texture_load(complete_x_prologues_tx, x_prologue_texel, vec4(0.0f));
  ...
  if (gl_LocalInvocationID.y == 0) {
    ...
    vec4 y_accumulated_color = texture_load(complete_y_prologues_tx, y_prologue_texel, vec4(0.0f));
