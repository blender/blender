...
void main()
{
  ...
  bool inner = all(lessThan(abs(pos), vec2(1.0f)));

  /* Scale the inner part of the border.
   * Add a sub pixel offset to the outer part to make sure we don't miss a pixel row/column. */
  vec2 final_pos = pos * ((inner) ? (1.0f - width) : 1.05f);

  uv = final_pos;
  /* Rescale to the corner size and position the corner. */
  if (corner_id == 0) {
    /* top right */
    final_pos = (final_pos - vec2(1.0f, 1.0f)) * scale + rect.yw;
  }
  else if (corner_id == 1) {
    /* top left */
    final_pos = (final_pos - vec2(-1.0f, 1.0f)) * scale + rect.xw;
  }
  else if (corner_id == 2) {
    /* bottom left */
    final_pos = (final_pos - vec2(-1.0f, -1.0f)) * scale + rect.xz;
  }
  else {
    /* bottom right */
    final_pos = (final_pos - vec2(1.0f, -1.0f)) * scale + rect.yz;
  }
