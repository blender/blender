...
vec2 do_widget()
{
  ...
  const vec2 ofs = vec2(0.5f, -0.5f);
  lineWidth = abs(rect.x - recti.x);
  vec2 emboss_ofs = vec2(0.0f, -lineWidth);
  ...
  borderColor = colorEdge * vec4(colorEdge.aaa, 1.0f);
  embossColor = colorEmboss * vec4(colorEmboss.aaa, 1.0f);
...
vec2 do_tria()
{
  ...
  if (triaType == 0.0f) {
    ...
    outRoundCorners = vec4(0.01f);
  }
  else if (triaType == 1.0f) {
    /* ROUNDBOX_TRIA_ARROWS */
    pos = arrow_pos[vidx];
    uvInterp = arrow_uvs[vidx];
    uvInterp -= vec2(0.05f, 0.63f); /* Translate */
    outRectSize = vec2(0.74f, 0.17f);
    outRoundCorners = vec4(0.08f);
  }
  else if (triaType == 2.0f) {
    /* ROUNDBOX_TRIA_SCROLL */
    pos = point_pos[vidx];
    uvInterp = point_uvs[vidx];
    outRectSize = vec2(1.0f);
    outRoundCorners = vec4(0.5f);
  }
  else if (triaType == 3.0f) {
    /* ROUNDBOX_TRIA_MENU */
    pos = tria2 ? vec2(0.0f) : arrow_pos[vidx]; /* Solo tria */
    pos = vec2(pos.y, -pos.x);                  /* Rotate */
    pos += vec2(-0.05f, 0.0f);                  /* Translate */
    size *= 0.8f;                               /* Scale */
    uvInterp = arrow_uvs[vidx];
    uvInterp -= vec2(0.05f, 0.63f); /* Translate */
    outRectSize = vec2(0.74f, 0.17f);
    ...
    pos = pos.x * vec2(0.0872f, -0.996f) + pos.y * vec2(0.996f, 0.0872f); /* Rotate (85deg) */
    pos += vec2(-0.1f, 0.2f);                                             /* Translate */
    center = tria1Center;
    size = tria1Size * 1.7f; /* Scale */
    uvInterp = arrow_uvs[vidx];
    uvInterp -= tria2 ? vec2(0.4f, 0.65f) : vec2(0.08f, 0.65f); /* Translate */
    outRectSize = vec2(0.74f, 0.14f);
    outRoundCorners = vec4(0.01f);
  }
    ...
    pos = tria2 ? vec2(0.0f) : arrow_pos[min(vidx, 2)];                 /* Only keep 1 triangle. */
    pos = pos.x * vec2(0.707f, 0.707f) + pos.y * vec2(-0.707f, 0.707f); /* Rotate (45deg) */
    pos += vec2(-1.7f, 2.4f); /* Translate (hard-coded, might want to remove). */
    size *= 0.4f;             /* Scale */
    uvInterp = arrow_uvs[vidx];
    uvInterp -= vec2(0.05f, 0.05f); /* Translate */
    outRectSize = vec2(0.75f);
    outRoundCorners = vec4(0.01f);
  }
    ...
    pos = point_pos[vidx];
    uvInterp = point_uvs[vidx];
    uvInterp -= vec2(0.2f, 0.45f); /* Translate */
    outRectSize = vec2(0.6f, 0.1f);
    outRoundCorners = vec4(0.01f);
  }
  ...
  innerColor = colorTria * vec4(colorTria.aaa, 1.0f);

  lineWidth = 0.0f;
  borderColor = vec4(0.0f);
  embossColor = vec4(0.0f);
