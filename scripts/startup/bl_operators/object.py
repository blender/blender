    ...
    def execute(self, context):
        ...
        if self.case_sensitive:
            ...
            def pattern_match(a, b):
                return fnmatch.fnmatchcase(a.upper(), b.upper())
...
class SelectHierarchy(Operator):
    ...
    def execute(self, context):
        ...
        if self.direction == "PARENT":
            ...
            for obj in selected_objects:
                select_new.extend(
                    [child for child in obj.children if child.visible_get()]
                )
...
class SubdivisionSet(Operator):
    ...
    relative: BoolProperty(
        name="Relative",
        description=(
            "Apply the subdivision surface level as an offset "
    ...
    def execute(self, context):
        ...
        def set_object_subd(obj):
            ...
            try:
                ...
                self.report(
                    {"WARNING"}, "Modifiers cannot be added to object: " + obj.name
                )
...
class ShapeTransfer(Operator):
    ...
    use_clamp: BoolProperty(
        name="Clamp Offset",
        description=(
            "Clamp the transformation to the distance each "
    ...
    def _main(self, ob_act, objects, mode="OFFSET", use_clamp=False):
        ...
        for ob_other in objects:
            if ob_other.type != "MESH":
                self.report(
                    {"WARNING"}, tip_("Skipping '%s', not a mesh") % ob_other.name
                )
                continue
            ...
            if mode == "OFFSET":
                for i, vert_cos in enumerate(median_coords):
                    vert_cos.append(
                        target_coords[i] + (orig_shape_coords[i] - orig_coords[i])
                    )
                ...
                for ed in me.edges:
                    ...
                    pt = barycentric_transform(
                        orig_shape_coords[i1], v2, v1, n1loc, v2_to, v1_to, n1loc_to
                    )
                    median_coords[i1].append(pt)

                    pt = barycentric_transform(
                        orig_shape_coords[i2], v1, v2, n2loc, v1_to, v2_to, n2loc_to
                    )
    ...
    def execute(self, context):
        ...
        if 1:  # swap from/to, means we can't copy to many at once.
            if len(objects) != 1:
                self.report(
                    {"ERROR"},
                    ("Expected one other selected " "mesh object to copy from"),
                )
...
class JoinUVs(Operator):
    ...
    def _main(self, context):
        ...
        if not mesh.uv_layers:
            self.report(
                {"WARNING"},
                tip_("Object: %s, Mesh: '%s' has no UVs") % (obj.name, mesh.name),
            )
        else:
...
class OBJECT_OT_assign_property_defaults(Operator):
    ...
    def poll(cls, context):
        obj = context.active_object
        return (
            obj is not None and obj.library is None and obj.mode in {"POSE", "OBJECT"}
        )
