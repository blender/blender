...
class AlphaDistanceFromObjectShader(CurveMappingModifier):
    ...
    def __init__(
        self, blend, influence, mapping, invert, curve, target, range_min, range_max
    ):
        CurveMappingModifier.__init__(self, blend, influence, mapping, invert, curve)
        if target is None:
...
class TangentThicknessShader(ThicknessBlenderMixIn, CurveMappingModifier):
    ...
    def shade(self, stroke):
        ...
        for svert in it:
            angle = angle_x_normal(it)
            thickness = (
                self.thickness.min + self.evaluate(angle / pi) * self.thickness.delta
            )
...
class NoiseShader:
    ...
    def noisegen(self, stroke, n1=Noise(), n2=Noise()):
        ...
        for svert in stroke:
            a = n1.turbulence_smooth(
                self.scale * svert.curvilinear_abscissa + initU1, 2
            )
            b = n2.turbulence_smooth(
                self.scale * svert.curvilinear_abscissa + initU2, 2
            )
            yield (svert, a, b)
...
class ColorNoiseShader(ColorRampModifier, NoiseShader):
    ...
    def shade(self, stroke):
        for svert, noiseval1, noiseval2 in self.noisegen(stroke):
            ...
            svert.attribute.color = self.blend_ramp(
                svert.attribute.color, self.evaluate(position)
            )
...
class AlphaNoiseShader(CurveMappingModifier, NoiseShader):
    ...
    def __init__(
        self, blend, influence, mapping, invert, curve, amplitude, period, seed=512
    ):
        CurveMappingModifier.__init__(self, blend, influence, mapping, invert, curve)
        NoiseShader.__init__(self, amplitude, period, seed)
    ...
    def shade(self, stroke, n1=Noise(), n2=Noise()):
        for svert, noiseval1, noiseval2 in self.noisegen(stroke):
            position = abs(noiseval1 + noiseval2)
            svert.attribute.alpha = self.blend(
                svert.attribute.alpha, self.evaluate(position)
            )
...
class CreaseAngleColorShader(ColorRampModifier):
    ...
    def shade(self, stroke):
        for svert in stroke:
            ...
            svert.attribute.color = self.blend_ramp(
                svert.attribute.color, self.evaluate(t)
            )
...
class Curvature3DAlphaShader(CurveMappingModifier):
    ...
    def __init__(
        self, blend, influence, mapping, invert, curve, curvature_min, curvature_max
    ):
        CurveMappingModifier.__init__(self, blend, influence, mapping, invert, curve)
        self.curvature = BoundedProperty(curvature_min, curvature_max)
...
class PerlinNoise1DShader(StrokeShader):
    ...
    def shade(self, stroke):
        ...
        for svert in stroke:
            nres = self.noise.turbulence1(
                length * svert.u, self.freq, self.amp, self.oct
            )
...
class Transform2DShader(StrokeShader):
    ...
    def __init__(self, pivot, scale_x, scale_y, angle, pivot_u, pivot_x, pivot_y):
        ...
        if pivot not in {"START", "END", "CENTER", "ABSOLUTE", "PARAM"}:
            raise ValueError(
                "expected pivot in {'START', 'END', 'CENTER', 'ABSOLUTE', 'PARAM'}, not"
                + pivot
            )
            ...
            if self.pivot_u < stroke[0].u:
                ...
                for prev, svert in pairwise(stroke):
                    if self.pivot_u < svert.u:
                        break
                pivot = svert.point + (svert.u - self.pivot_u) * (
                    prev.point - svert.point
                )
...
def process(layer_name, lineset_name):
    ...
    if lineset.select_by_collection:
        if lineset.collection is not None:
            names = {
                getQualifiedObjectName(ob): True
                for ob in get_grouped_objects(lineset.collection)
            }
    ...
    if linestyle.use_chaining:
        ...
        Operators.chain(
            ChainPredicateIterator(FalseUP1D(), FalseBP1D()), NotUP1D(upred)
        )
    # split chains
    ...
    if linestyle.use_sorting:
        integration = integration_types.get(
            linestyle.integration_type, IntegrationType.MEAN
        )
    ...
    for m in linestyle.geometry_modifiers:
        ...
        if m.type == "SAMPLING":
            ...
            if m.shape == "CIRCLES":
                shaders_list.append(
                    pyBluePrintCirclesShader(m.rounds, m.random_radius, m.random_center)
                )
    ...
    if (not linestyle.use_chaining) or (
        linestyle.chaining == "PLAIN" and linestyle.use_same_object
    ):
    ...
    shaders_list.append(
        BaseThicknessShader(
            linestyle.thickness, thickness_position, linestyle.thickness_ratio
        )
    )
    # -- Modifiers -- #
    for m in linestyle.color_modifiers:
        ...
        if m.type == "ALONG_STROKE":
            ...
            shaders_list.append(
                ColorDistanceFromCameraShader(
                    m.blend, m.influence, m.color_ramp, m.range_min, m.range_max
                )
            )
        elif m.type == "DISTANCE_FROM_OBJECT":
            if m.target is not None:
                shaders_list.append(
                    ColorDistanceFromObjectShader(
                        m.blend,
                        m.influence,
                        m.color_ramp,
                        m.target,
            ...
            shaders_list.append(
                ColorMaterialShader(
                    m.blend, m.influence, m.color_ramp, m.material_attribute, m.use_ramp
                )
            )
        elif m.type == "TANGENT":
            shaders_list.append(TangentColorShader(m.blend, m.influence, m.color_ramp))
        elif m.type == "CREASE_ANGLE":
            shaders_list.append(
                CreaseAngleColorShader(
                    m.blend, m.influence, m.color_ramp, m.angle_min, m.angle_max
                )
            )
        elif m.type == "CURVATURE_3D":
            shaders_list.append(
                Curvature3DColorShader(
                    m.blend, m.influence, m.color_ramp, m.curvature_min, m.curvature_max
                )
            )
        elif m.type == "NOISE":
            shaders_list.append(
                ColorNoiseShader(
                    m.blend, m.influence, m.color_ramp, m.amplitude, m.period, m.seed
                )
            )
    for m in linestyle.alpha_modifiers:
        ...
        if m.type == "ALONG_STROKE":
            shaders_list.append(
                AlphaAlongStrokeShader(
                    m.blend, m.influence, m.mapping, m.invert, m.curve
                )
            )
        elif m.type == "DISTANCE_FROM_CAMERA":
            shaders_list.append(
                AlphaDistanceFromCameraShader(
                    m.blend,
                    m.influence,
                    m.mapping,
                    m.invert,
    ...
    for m in linestyle.thickness_modifiers:
        ...
        if m.type == "ALONG_STROKE":
            ...
            shaders_list.append(
                ThicknessDistanceFromCameraShader(
                    thickness_position,
                    linestyle.thickness_ratio,
                    m.blend,
                    m.influence,
