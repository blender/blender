...
def gather_action_armature_sampled(armature_uuid: str,
    ...
    try:
        channels, additional_channels = __gather_channels(
    ...
    if export_settings['gltf_export_extra_animations']:
        for chan in [chan for chan in additional_channels.values() if len(chan['properties']) != 0]:
            for channel_group_name, channel_group in chan['properties'].items():
                ...
                if sampler is not None:
                    additional_samplers.append((channel_group_name, sampler))

    if not channels:
        return None, additional_samplers
    ...
    return channels, additional_samplers
