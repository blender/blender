...
static void clear_temp_flags(BMesh *bm)
{
	...
	for(v = BMIter_New(&verts, bm, BM_VERTS, bm ); v; v = BMIter_Step(&verts)) bmesh_clear_sysflag(&(v->head), BMESH_TEMP_FLAGS);
	for(e = BMIter_New(&edges, bm, BM_EDGES, bm ); e; e = BMIter_Step(&edges)) bmesh_clear_sysflag(&(e->head), BMESH_TEMP_FLAGS);
	for(f = BMIter_New(&faces, bm, BM_FACES, bm ); f; f = BMIter_Step(&faces)){
		bmesh_clear_sysflag(&(f->head), BMESH_TEMP_FLAGS);
		for(l = BMIter_New(&loops, bm, BM_LOOPS_OF_FACE, f ); l; l = BMIter_Step(&loops)) bmesh_clear_sysflag(&(l->head), BMESH_TEMP_FLAGS);
	}
}
