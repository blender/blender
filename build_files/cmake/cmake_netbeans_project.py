...
def create_nb_project_main():
    ...
    if SIMPLE_PROJECTFILE:
        ...
        includes = list(set(includes) | {dirname(f) for f in files if is_c_header(f)})
        includes.sort()
        ...
        defines += [
            cdef.replace("#define", "").strip() for cdef in cmake_compiler_defines()
        ]
        ...
        f.write(
            "                <sourceRootElem>%s</sourceRootElem>\n" % SOURCE_DIR
        )  # base_root_rel
        f.write("            </sourceRootList>\n")
        f.write("            <confList>\n")
        f.write("                <confElem>\n")
        ...
        # write files!
        files_rel_local = [
            normpath(relpath(join(CMAKE_DIR, path), SOURCE_DIR)) for path in files_rel
        ]
        ...
        f.write(
            r"  <sourceFolderFilter>^(nbproject|__pycache__|.*\.py|.*\.html|.*\.blend)$</sourceFolderFilter>\n"
        )
