...
Distance::Distance(double accuracy, unsigned int maximum_iterations):
...
#if 1
	computeChi(init_pose);
#else
	m_chi(2) = 0.5;
#endif
	return ConstraintSet::initialise(init_pose);
}
...
void Distance::updateControlOutput(const Timestamp& timestamp)
{
    ...
    m_ydot(0)=m_yddot+m_K*(m_yd-m_chi(2));
}
