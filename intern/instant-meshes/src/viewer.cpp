      ...
      mBVH(nullptr)
{
  ...
  mFlowLineShader.init(
      "flowline_shader", (const char *)shader_flowline_vert, (const char *)shader_flowline_frag);

  mStrokeShader.init(
      "stroke_shader", (const char *)shader_flowline_vert, (const char *)shader_flowline_frag);
  ...
  mCreaseAngleSlider->setCallback(
      [&](Float value) { mCreaseAngleBox->setValue(std::to_string((int)(value * 90))); });
  ...
  mLayers[OrientationField] = new CheckBox(
      orientFieldPanel, "Orientation field (n-RoSy)", layerCB);
  ...
  mLayers[PositionFieldSingularities] = new CheckBox(
      posFieldSingPanel, "Position field singularities", layerCB);
  ...
  mSymmetryBox = new ComboBox(
      window,
      {"Triangles (6-RoSy, 6-PoSy)", "Quads (2-RoSy, 4-PoSy)", "Quads (4-RoSy, 4-PoSy)"},
  ...
  mSymmetryBox->setCallback([&](int index) {
    ...
    if (index == 0) {
      ...
      mOptimizer.setPoSy(
          3);  // TODO this should be referred to as 6 consistently (just cosmetic change)
    }
    else if (index == 1) {
  ...
  mExtrinsicBox->setTooltip(
      "Use an extrinsic smoothness energy with "
  ...
  mCreaseBox->setTooltip(
      "Don't smooth discontinuous surface "
  ...
  //" New comb and contour strokes cannot be added after using the singularity attractor and scare
  //brush." " If you continue, singularity adjustments will be discarded in favor of new stroke
  //annotations.", "Continue", "Cancel", true);
  //});
  ...
  orientTools->setLayout(new BoxLayout(Orientation::Horizontal, Alignment::Middle, 0, 6));
  ...
  uvTools->setLayout(new BoxLayout(Orientation::Horizontal, Alignment::Middle, 0, 6));
  ...
  mSmoothSlider->setCallback(
      [&](Float value) { mSmoothBox->setValue(std::to_string((int)(value * 10))); });
  ...
  mSwitchBtn->setCallback([&]() { keyboardEvent(GLFW_KEY_BACKSLASH, 0, GLFW_PRESS, 0); });
...
void Viewer::draw(NVGcontext *ctx)
{
  if (mRes.levels() == 0) {
    ...
    NVGpaint imgPaint = nvgImagePattern(
        ctx, (mSize[0] - size) / 2, (mSize[1] - size) / 2, size, size, 0, appIcon, 1.0f);
    nvgBeginPath(ctx);
...
void Viewer::setSymmetry(int rosy, int posy)
{
  if (rosy == 6 && posy == 3)
    ...
    throw std::runtime_error(
        "Selected RoSy/PoSy combination is not supported by the user interface");
...
void Viewer::setCreaseAnglePrompt(bool enabled, Float value)
{
  ...
  msg->setCallback([&, value, enabled](int button) {
    if (button == 0) {
      ...
      Vector3f savedTranslation = mCamera.modelTranslation;
      loadInput(mFilename,
                enabled ? (value < 0 ? 20 : value) : (Float)-1.0f,
                mRes.scale(),
                -1,
                -1,
                mOptimizer.rosy(),
...
Vector3f compat_uv(const Vector3f &o,
                   const Vector3f &ref,
                   const Vector3f &q,
                   const Vector3f &t,
                   Float scale,
...
void Viewer::setFloorPosition()
{
  ...
  Vector3f floorCenter = Vector3f(
      .5f * (aabb.max.x() + aabb.min.x()), aabb.min.y(), .5f * (aabb.max.z() + aabb.min.z()));
...
void Viewer::renderMitsuba()
{
  ...
  if (mLayers[InputMesh]->checked()) {
    ...
    if (mVisualizeBox->selectedIndex() == 1) {
      {
        std::vector<MatrixXf> V_props = {
            std::move(V), std::move(N), std::move(Nd), std::move(C), std::move(Q), std::move(O)};
          ...
          [&](const tbb::blocked_range<uint32_t> &range) {
            for (uint32_t f = range.begin(); f != range.end(); ++f) {
              ...
              for (int i = 1; i < 3; ++i) {
                ...
                o[i] = compat_position(
                           v[0], fn, q[0], o[0], v[i], fn, q[i], o[i], scale, inv_scale)
                           .second;
              }

              for (int i = 0; i < 3; ++i) {
                Vector3f rel = v[i] - o[i];
                UV.col(f * 3 + i) = Vector2f(rel.dot(q[i]) * inv_scale,
  ...
  if (mLayers[InputMesh]->checked()) {
    ...
    if (mVisualizeBox->selectedIndex() == 1) {
      of << "\t\t\t<texture name=\"diffuseReflectance\" type=\"posyfield\">" << endl
         ...
         << "\t\t\t\t<srgb name=\"interiorColor\" value=\"" << base_color.transpose() << "\"/>"
         << endl
         << "\t\t\t\t<srgb name=\"edgeColor0\" value=\"" << edge_color0.transpose() << "\"/>"
         << endl
         << "\t\t\t\t<srgb name=\"edgeColor1\" value=\"" << edge_color1.transpose() << "\"/>"
         << endl
         << "\t\t\t\t<srgb name=\"edgeColor2\" value=\"" << edge_color2.transpose() << "\"/>"
         << endl
         << "\t\t\t</texture>" << endl;
    }
      ...
      of << "\t\t\t<texture name=\"diffuseReflectance\" type=\"wireframe\">" << endl
         ...
         << "\t\t\t\t<srgb name=\"interiorColor\" value=\"" << base_color.transpose() << "\"/>"
         << endl
         << "\t\t\t\t<srgb name=\"edgeColor\" value=\"" << edge_color0.transpose() << "\"/>"
         << endl
         << "\t\t\t</texture>" << endl;
    }
    else {
      of << "\t\t\t<srgb name=\"diffuseReflectance\" value=\"" << base_color.transpose() << "\"/>"
         << endl;
    }
  ...
  if (mLayers[OutputMesh]->checked() && mF_extracted.size() > 0) {
    ...
    if (mLayers[OutputMeshWireframe]->checked()) {
      ...
      of << "\t\t\t\t<texture name=\"diffuseReflectance\" type=\"wireframe\">" << endl
         ...
         << "\t\t\t\t\t<srgb name=\"interiorColor\" value=\"" << finalmesh_color.transpose()
         << "\"/>" << endl
         << "\t\t\t\t\t<srgb name=\"edgeColor\" value=\"0 0 0\"/>" << endl
         << "\t\t\t\t\t<boolean name=\"quads\" value=\"" << (posy == 4 ? "true" : "false")
         << "\"/>" << endl
         << "\t\t\t\t</texture>" << endl;
    }
    else {
      of << "\t\t\t\t<srgb name=\"diffuseReflectance\" value=\"" << finalmesh_color.transpose()
         << "\"/>" << endl;
    }
    of << "\t\t\t</bsdf>" << endl << "\t\t</bsdf>" << endl << "\t</shape>" << endl;
  }
  ...
  if (mLayers[OrientationFieldSingularities]->checked()) {
    ...
    for (auto &sing : mOrientationSingularities) {
      ...
      of << "\t<shape type=\"sphere\">" << endl
         << "\t\t<point name=\"center\" x=\"" << p.x() << "\" y=\"" << p.y() << "\" z=\"" << p.z()
         << "\"/>" << endl
  ...
  if (mLayers[PositionFieldSingularities]->checked()) {
    ...
    for (auto &sing : mPositionSingularities) {
      ...
      of << "\t<shape type=\"sphere\" id=\"pos_sing_" << ctr++ << "\">" << endl
         ...
         << "\t\t\t\t<point name=\"position\" x=\"" << p.x() << "\" y=\"" << p.y() << "\" z=\""
         << p.z() << "\"/>" << endl
         << "\t\t\t\t<vector name=\"dir0\" x=\"" << dir0.x() << "\" y=\"" << dir0.y() << "\" z=\""
         << dir0.z() << "\"/>" << endl
         << "\t\t\t\t<vector name=\"dir1\" x=\"" << dir1.x() << "\" y=\"" << dir1.y() << "\" z=\""
         << dir1.z() << "\"/>" << endl
         << "\t\t\t\t<vector name=\"normal\" x=\"" << n.x() << "\" y=\"" << n.y() << "\" z=\""
         << n.z() << "\"/>" << endl
         << "\t\t\t\t<srgb name=\"color\" value=\""
         << ((shift.array().abs() <= 1).all() ? yellow : orange).transpose() << "\"/>" << endl
         << "\t\t\t</texture>" << endl
         << "\t\t</bsdf>" << endl
         << "\t</shape>" << endl
         << endl;
...
void Viewer::extractConsensusGraph()
{
      ...
      [&](const tbb::blocked_range<uint32_t> &range) {
        for (uint32_t i = range.begin(); i < range.end(); ++i) {
          for (Link *link = adj[i]; link != adj[i + 1]; ++link) {
            ...
            std::pair<Vector2i, Vector2i> shift = compat_position(V.col(i),
                                                                  N.col(i),
                                                                  Q_rot.first,
                                                                  O.col(i),
                                                                  V.col(j),
  ...
  mOutputMeshWireframeShader.uploadAttrib(
      "position", MatrixXf(outputMeshWireframe.block(0, 0, 3, mOutputMeshLines)));
  mOutputMeshWireframeShader.uploadAttrib(
  ...
  tbb::parallel_for(
      ...
      [&](const tbb::blocked_range<uint32_t> &range) {
        ...
        for (uint32_t k = range.begin(); k != range.end(); ++k) {
          ...
          Vector4u8 cvalue;
          cvalue << (hsv_to_rgb(hue, 0.5f, 0.5f * rng.nextFloat() + 0.5f) * 255).cast<uint8_t>(),
              (uint8_t)0x0;
          int nTries = 0;
          ...
          do {
            ...
            for (uint32_t step = 0; step < nSteps; ++step) {
              ...
              Float thicknessP = (1 - std::pow((2 * ((step + 1) / (Float)nSteps) - 1), 2.0f)) *
                                 thickness;
      ...
      if (level >= 0) {
        ...
        for (auto phase : phases) {
          ...
          for (uint32_t i : phase) {
            color << (hsv_to_rgb(hue, 0.5f, 0.5f * rng.nextFloat() + 0.5f) * 255).cast<uint8_t>(),
                (uint8_t)255;
            Cp.col(i) = color;
          }
        }
...
{
  if (mRes.iterationsQ() == mOrientationSingularityShader.attribVersion("position") ||
      mRes.iterationsQ() < 0)
    return false;

  compute_orientation_singularities(
      mRes, mOrientationSingularities, mExtrinsicBox->checked(), mOptimizer.rosy());
  ...
  const Vector3f red = Vector3f::UnitX(), green = Vector3f::UnitY(), blue = Vector3f::UnitZ();
...
{
  if (mRes.iterationsO() == mPositionSingularityShader.attribVersion("position") ||
      mRes.iterationsO() < 0)
    return false;

  compute_position_singularities(mRes,
                                 mOrientationSingularities,
                                 mPositionSingularities,
                                 mExtrinsicBox->checked(),
                                 mOptimizer.rosy(),
                                 mOptimizer.posy());
...
void Viewer::drawContents()
{
  ...
  if (canRefresh &&
      (mLayers[OrientationFieldSingularities]->checked() || mOrientationAttractor->pushed()) &&
      refreshOrientationSingularities())
    repaint();

  if (canRefresh &&
      (mLayers[PositionFieldSingularities]->checked() || mPositionAttractor->pushed()) &&
  ...
  if (canRefresh &&
      (mLayers[OrientationField]->checked() ||
       (mLayers[InputMesh]->checked() && mVisualizeBox->selectedIndex() == 1)) &&
  ...
  drawFunctor[InputMesh] = [&](uint32_t offset, uint32_t count) {
    ...
    bool show_uvs = mMeshShader44.attribVersion("uv") > 0 && mVisualizeBox->selectedIndex() == 1;
    shader->bind();
  ...
  drawFunctor[OrientationField] = [&](uint32_t offset, uint32_t count) {
    ...
    mOrientationFieldShader.setUniform("offset",
                                       (Float)mMeshStats.mAverageEdgeLength * (1.0f / 5.0f));
  ...
  drawFunctor[PositionFieldSingularities] = [&](uint32_t offset, uint32_t count) {
    ...
    mPositionSingularityShader.setUniform(
        "point_size",
        mRes.scale() * 0.4f * std::pow((Float)2, mPositionFieldSingSizeSlider->value() * 4 - 2));
  ...
  drawFunctor[FaceLabels] = [&](uint32_t offset, uint32_t count) {
    ...
    for (uint32_t i = offset; i < offset + count; ++i) {
      Vector4f pos;
      pos << (1.0f / 3.0f) * (V.col(F(0, i)) + V.col(F(1, i)) + V.col(F(2, i))).cast<float>(),
          1.0f;
...
void Viewer::refreshStrokes()
{
  ...
  for (auto const &stroke : mStrokes) {
    ...
    for (uint32_t i = 1; i < curve.size(); ++i) {
      ...
      Float thicknessP = (1 - std::pow((2 * ((i + 1) / (Float)curve.size()) - 1), 2.0f)) *
                         thickness;
...
void Viewer::drawOverlay()
{
  ...
  for (auto const &stroke : mStrokes) {
    ...
    if (!mBVH->rayIntersect(Ray(pos.head<3>(), (civ - pos).head<3>(), 0.0f, 1.0f))) {
      NVGpaint imgPaint = nvgImagePattern(ctx, coord.x(), coord.y(), 32, 32, 0, delIcon, 0.6f);
      nvgBeginPath(ctx);
...
bool Viewer::mouseMotionEvent(const Vector2i &p, const Vector2i &rel, int button, int modifiers)
{
  ...
  if (!Screen::mouseMotionEvent(p, rel, button, modifiers)) {
    if (mCamera.arcball.motion(p)) {
      ...
      computeCameraMatrices(model, view, proj);
      float zval =
          project(mMeshStats.mWeightedCenter.cast<float>(), view * model, proj, mSize).z();
      Eigen::Vector3f pos1 = unproject(
          Eigen::Vector3f(p.x(), mSize.y() - p.y(), zval), view * model, proj, mSize);
      Eigen::Vector3f pos0 = unproject(
          Eigen::Vector3f(mTranslateStart.x(), mSize.y() - mTranslateStart.y(), zval),
          view * model,
          proj,
          mSize);
    ...
    if (toolActive()) {
      ...
      if (drag) {
        ...
        for (uint32_t i = 0; i < mScreenCurve.size(); ++i) {
          Eigen::Vector3f pos1 = unproject(
              Eigen::Vector3f(mScreenCurve[i].x(), mSize.y() - mScreenCurve[i].y(), 0.0f),
              view * model,
              proj,
              mSize);
          Eigen::Vector3f pos2 = unproject(
              Eigen::Vector3f(mScreenCurve[i].x(), mSize.y() - mScreenCurve[i].y(), 1.0f),
              view * model,
              proj,
              mSize);
...
void Viewer::loadInput(std::string filename,
                       Float creaseAngle,
                       Float scale,
                       int face_count,
                       int vertex_count,
                       ...
                       int knn_points)
{
  ...
  if (!pointcloud) {
    ...
    Float max = std::log(2 * mRes.V().cols());
    if (unsafe < max)
      mScaleSlider->setHighlightedRange(std::make_pair((unsafe - min) / (max - min), 1.f));
  }
...
void Viewer::shareGLBuffers()
{
  ...
  for (auto sh :
       {&mMeshShader24, &mMeshShader63, &mPointShader24, &mPointShader44, &mPointShader63}) {
    sh->bind();
    ...
    if (sh->name().find("mesh") != std::string::npos && mRes.F().size() > 0) {
      sh->shareAttrib(mMeshShader44,
                      mMeshShader44.hasAttrib("normal_data") ? "normal_data" : "normal",
                      "normal_data");
