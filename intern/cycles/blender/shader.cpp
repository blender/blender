...
void BlenderSync::find_shader(const ::ID *id, array<Node *> &used_shaders, Shader *default_shader)
{
...
static DisplacementMethod get_displacement_method(::Material &b_mat)
{
...
static ImageAlphaType get_image_alpha_type(::Image &b_image)
{
...
int blender_attribute_name_split_type(ustring name, string *r_real_name)
{
  ...
  if (sname.substr(0, object_attr_prefix.size()) == object_attr_prefix) {
    *r_real_name = sname.substr(object_attr_prefix.size());
    return SHD_ATTRIBUTE_OBJECT;
  }

  if (sname.substr(0, instancer_attr_prefix.size()) == instancer_attr_prefix) {
    *r_real_name = sname.substr(instancer_attr_prefix.size());
    return SHD_ATTRIBUTE_INSTANCER;
  }

  if (sname.substr(0, view_layer_attr_prefix.size()) == view_layer_attr_prefix) {
    *r_real_name = sname.substr(view_layer_attr_prefix.size());
    return SHD_ATTRIBUTE_VIEW_LAYER;
  }

  return SHD_ATTRIBUTE_GEOMETRY;
}

/* Graph */

static float3 get_node_output_rgba(::bNode &b_node, const string &name)
{
  ::bNodeSocket *b_sock = b_node.output_by_identifier(name);
  BLI_assert(b_sock->type == SOCK_RGBA);
  const auto &default_value = *b_sock->default_value_typed<bNodeSocketValueRGBA>();
  return make_float3(default_value.value[0], default_value.value[1], default_value.value[2]);
}

static float get_node_output_value(::bNode &b_node, const string &name)
{
  ::bNodeSocket *b_sock = b_node.output_by_identifier(name);
  BLI_assert(b_sock->type == SOCK_FLOAT);
  const auto &default_value = *b_sock->default_value_typed<bNodeSocketValueFloat>();
  return default_value.value;
}

static float3 get_node_output_vector(::bNode &b_node, const string &name)
{
  ::bNodeSocket *b_sock = b_node.output_by_identifier(name);
  BLI_assert(b_sock->type == SOCK_VECTOR);
  const auto &default_value = *b_sock->default_value_typed<bNodeSocketValueVector>();
  return make_float3(default_value.value[0], default_value.value[1], default_value.value[2]);
}

static SocketType::Type convert_socket_type(::bNodeSocket &b_socket)
{
  switch (eNodeSocketDatatype(b_socket.type)) {
    case SOCK_FLOAT:
                              ...
                              ::ID &b_id)
{
  ...
  switch (input->type()) {
    ...
    case SocketType::INT: {
      if (b_sock.type == SOCK_BOOLEAN) {
        const auto &default_value = *b_sock.default_value_typed<bNodeSocketValueBoolean>();
        ...
        const auto &default_value = *b_sock.default_value_typed<bNodeSocketValueInt>();
        node->set(socket, default_value.value);
      }
      break;
    }
    ...
    case SocketType::STRING: {
      const auto &default_value = *b_sock.default_value_typed<bNodeSocketValueString>();
...
static void get_tex_mapping(TextureNode *mapping, const ::TexMapping *b_mapping)
{
  if (!b_mapping) {
    return;
  }
...
static bool is_image_animated(eImageSource b_image_source, ::ImageUser &b_image_user)
{
  return (b_image_source == IMA_SRC_MOVIE || b_image_source == IMA_SRC_SEQUENCE) &&
         (b_image_user.flag & IMA_ANIM_ALWAYS) != 0;
}
                            ...
                            ::bNode &b_node)
{
  ...
  if (b_node.is_type("ShaderNodeRGBCurve")) {
    const auto &mapping = *static_cast<CurveMapping *>(b_node.storage);
    ...
    curves->set_extrapolate((mapping.flag & CUMA_EXTEND_EXTRAPOLATE) != 0);
    node = curves;
  }
  if (b_node.is_type("ShaderNodeVectorCurve")) {
    ...
    curve->set_extrapolate((mapping.flag & CUMA_EXTEND_EXTRAPOLATE) != 0);
    node = curve;
  }
  else if (b_node.is_type("ShaderNodeValToRGB")) {
    RGBRampNode *ramp = graph->create_node<RGBRampNode>();
    const auto &b_color_ramp = *static_cast<::ColorBand *>(b_node.storage);
    ...
    ramp->set_interpolate(b_color_ramp.ipotype != COLBAND_INTERP_CONSTANT);
    node = ramp;
  }
    ...
    mix->set_use_clamp(b_node.custom2 & SHD_MIXRGB_CLAMP);
    node = mix;
  }
  else if (b_node.is_type("ShaderNodeMix")) {
    const auto &storage = *static_cast<NodeShaderMix *>(b_node.storage);
    if (storage.data_type == SOCK_VECTOR) {
    ...
    const auto &storage = *static_cast<NodeCombSepColor *>(b_node.storage);
    ...
    const auto &storage = *static_cast<NodeMapRange *>(b_node.storage);
    if (storage.data_type == CD_PROP_FLOAT3) {
    ...
    const auto &storage = *static_cast<NodeShaderVectTransform *>(b_node.storage);
    ...
    const auto &storage = *static_cast<NodeShaderAttribute *>(b_node.storage);
    ...
    switch (b_node.custom1) {
      ...
      case SHD_GLOSSY_GGX:
        ...
        break;
      case SHD_GLOSSY_ASHIKHMIN_SHIRLEY:
    ...
    switch (b_node.custom1) {
      case SHD_HAIR_REFLECTION:
        hair->set_component(CLOSURE_BSDF_HAIR_REFLECTION_ID);
        break;
      case SHD_HAIR_TRANSMISSION:
    ...
    const auto &storage = *static_cast<NodeShaderHairPrincipled *>(b_node.storage);
    ...
    const auto &storage = *static_cast<NodeShaderVertexColor *>(b_node.storage);
...
#ifdef WITH_OSL
    const auto &storage = *static_cast<NodeShaderScript *>(b_node.storage);
    ...
    const auto &storage = *static_cast<NodeTexImage *>(b_node.storage);
    ::Image *b_image = blender::id_cast<::Image *>(b_node.id);
    ::ImageUser &b_image_user = const_cast<::ImageUser &>(storage.iuser);
    ...
    const ::TexMapping &b_texture_mapping = storage.base.tex_mapping;
    get_tex_mapping(image, &b_texture_mapping);

    if (b_image) {
      const ::eImageSource b_image_source = eImageSource(b_image->source);
      PointerRNA image_rna_ptr = RNA_id_pointer_create(&b_image->id);
      PointerRNA colorspace_ptr = RNA_pointer_get(&image_rna_ptr, "colorspace_settings");
    ...
    const auto &storage = *static_cast<NodeTexEnvironment *>(b_node.storage);
    ::Image *b_image = blender::id_cast<::Image *>(b_node.id);
    ::ImageUser &b_image_user = const_cast<::ImageUser &>(storage.iuser);
    ...
    if (b_image) {
      ...
      if (is_builtin) {
        ...
        env->handle = scene->image_manager->add_image(
            make_unique<BlenderImageLoader>(
                b_image, &b_image_user, image_frame, 0, (b_engine.flag & RE_ENGINE_PREVIEW) != 0),
            env->image_params());
      }
      else {
        env->set_filename(
    ...
    const auto &storage = *static_cast<NodeTexGradient *>(b_node.storage);
    ...
    const auto &storage = *static_cast<NodeTexVoronoi *>(b_node.storage);
    ...
    const auto &storage = *static_cast<NodeTexMagic *>(b_node.storage);
    ...
    const auto &storage = *static_cast<NodeTexWave *>(b_node.storage);
    ...
    const auto &storage = *static_cast<NodeTexChecker *>(b_node.storage);
    ...
    const auto &storage = *static_cast<NodeTexBrick *>(b_node.storage);
    ...
    const auto &storage = *static_cast<NodeTexNoise *>(b_node.storage);
    ...
    const auto &storage = *static_cast<NodeTexGabor *>(b_node.storage);
    ...
    if (const ::ID *b_object = b_node.id) {
      tex_coord->set_use_transform(true);
      tex_coord->set_ob_tfm(
          get_transform(blender::id_cast<const ::Object *>(b_object)->object_to_world()));
    }
    node = tex_coord;
  }
  else if (b_node.is_type("ShaderNodeTexSky")) {
    const auto &storage = *static_cast<NodeTexSky *>(b_node.storage);
    ...
    const auto &storage = *static_cast<NodeShaderNormalMap *>(b_node.storage);
    ...
    const auto &storage = *static_cast<NodeRadialTiling *>(b_node.storage);
    ...
    const auto &storage = *static_cast<NodeShaderTangent *>(b_node.storage);
    ...
    const auto &storage = *static_cast<NodeShaderUVMap *>(b_node.storage);
    ...
    const auto &storage = *static_cast<NodeShaderOutputAOV *>(b_node.storage);
...
static ShaderInput *node_find_input_by_name(const ::bNode &b_node,
                                            ShaderNode *node,
...
static ShaderOutput *node_find_output_by_name(::bNode &b_node,
                                              ShaderNode *node,
                                              ::bNodeSocket &b_socket)
{
  string name = b_socket.identifier;
                              ...
                              const ProxyMap &proxy_output_map)
{
  ...
  const bNode *output_node = ntreeShaderOutputNode(&b_ntree, SHD_OUTPUT_CYCLES);

  /* add nodes */
  for (::bNode *b_node : b_ntree.all_nodes()) {
    if (b_node->is_muted() || b_node->is_reroute()) {
      /* replace muted node with internal links */
      for (::bNodeLink &b_link : b_node->runtime->internal_links) {
        ::bNodeSocket *to_socket = b_link.tosock;
      ...
      ::bNodeTree *b_group_ntree = blender::id_cast<::bNodeTree *>(b_node->id);
       ...
       */
      for (::bNodeSocket *b_input : b_node->input_sockets()) {
      ...
      for (::bNodeSocket *b_output : b_node->output_sockets()) {
  ...
  /* connect nodes */
  for (::bNodeLink *b_link : b_ntree.all_links()) {
    /* Ignore invalid links to avoid unwanted cycles created in graph.
     * Also ignore links with unavailable sockets. */
    if (!((b_link->flag & NODE_LINK_VALID) != 0 && b_link->fromsock->is_available() &&
    ...
    ::bNodeSocket *b_from_sock = b_link->fromsock;
    ::bNodeSocket *b_to_sock = b_link->tosock;
                                                ...
                                                ::Depsgraph &b_depsgraph)
{
  ...
  for (ShaderNode *node : graph->nodes) {
    if (node->is_a(AttributeNode::get_node_type())) {
      ...
      if (type == SHD_ATTRIBUTE_VIEW_LAYER) {
        /* Look up the value. */
        const ::ViewLayer *b_layer = DEG_get_evaluated_view_layer(&b_depsgraph);
        const ::Scene *b_scene = DEG_get_evaluated_scene(&b_depsgraph);
        float4 value;
...
bool BlenderSync::scene_attr_needs_recalc(Shader *shader, ::Depsgraph &b_depsgraph)
{
  if (shader && shader_map.test_flag(shader, SHADER_WITH_LAYER_ATTRS)) {
    ::Scene *scene = DEG_get_evaluated_scene(&b_depsgraph);

    return shader_map.check_recalc(&scene->id) ||
           shader_map.check_recalc(reinterpret_cast<ID *>(scene->world)) ||
           shader_map.check_recalc(reinterpret_cast<ID *>(scene->camera));
  }
...
void BlenderSync::sync_materials(::Depsgraph &b_depsgraph, bool update_all)
{
  ...
  ITER_BEGIN (
      DEG_iterator_ids_begin, DEG_iterator_ids_next, DEG_iterator_ids_end, &data, ID *, b_id)
  {
    if (GS(b_id->name) != ID_MA) {
      continue;
    }

    ::Material &b_mat = blender::id_cast<::Material &>(*b_id);
    Shader *shader;
        ...
        scene_attr_needs_recalc(shader, b_depsgraph))
    {
      ...
      /* settings */
      PointerRNA mat_rna_ptr = RNA_id_pointer_create(&b_mat.id);
      PointerRNA cmat = RNA_pointer_get(&mat_rna_ptr, "cycles");
      shader->set_emission_sampling_method(get_emission_sampling(cmat));
      shader->set_use_transparent_shadow(b_mat.blend_flag & MA_BL_TRANSPARENT_SHADOW);
...
void BlenderSync::sync_world(::Depsgraph &b_depsgraph,
                             ::bScreen *b_screen,
                             ::View3D *b_v3d,
                             bool update_all)
{
      ...
      scene_attr_needs_recalc(shader, b_depsgraph))
  {
    ...
    if (b_world) {
      PointerRNA world_rna_ptr = RNA_id_pointer_create(&b_world->id);
      PointerRNA cvisibility = RNA_pointer_get(&world_rna_ptr, "cycles_visibility");
      uint visibility = 0;
...
void BlenderSync::sync_lights(::Depsgraph &b_depsgraph, bool update_all)
{
  ...
  ITER_BEGIN (
      DEG_iterator_ids_begin, DEG_iterator_ids_next, DEG_iterator_ids_end, &data, ID *, b_id)
  {
    if (GS(b_id->name) != ID_LA) {
      continue;
    }

    ::Light &b_light = blender::id_cast<::Light &>(*b_id);
    Shader *shader;
...
void BlenderSync::sync_shaders(::Depsgraph &b_depsgraph,
                               ::bScreen *b_screen,
                               ::View3D *b_v3d,
                               bool update_all)
{
