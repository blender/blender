...
class CYCLES_OT_use_shading_nodes(Operator):
    ...
    def poll(cls, context):
        return (
            getattr(context, "material", False)
            or getattr(context, "world", False)
            or getattr(context, "light", False)
        )
        ...
        if in_filepath != "":
            ...
            for frame in range(scene.frame_start, scene.frame_end + 1):
                ...
                if not os.path.isfile(filepath):
                    ...
                    err_msg = (
                        tip_("Frame '%s' not found, animation must be complete")
                        % filepath
                    )
