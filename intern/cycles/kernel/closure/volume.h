...
ccl_device_forceinline float volume_phase_widen_dD(const KernelGlobals kg,
                                                   const float prev_dD,
                                                   const float sampled_roughness)
{
  return max(prev_dD, kernel_data.integrator.differential_widen_scale * sampled_roughness);
}
