                                                              ...
                                                              const float3 to_P)
{
  ...
  const uint32_t visibility = SHADOW_CATCHER_PATH_VISIBILITY(path_flag, PATH_RAY_VISIBILITY_ALL);
...
ccl_device void integrator_volume_stack_init(KernelGlobals kg, IntegratorState state)
{
  ...
  const uint32_t path_flag = INTEGRATOR_STATE(state, path, flag);
  const uint32_t visibility = SHADOW_CATCHER_PATH_VISIBILITY(path_flag,
