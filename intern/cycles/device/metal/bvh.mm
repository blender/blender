                               ...
                               bool refit)
{
  ...
  id<MTLBuffer> sizeBuf = [mtl_device newBufferWithLength:8 options:MTLResourceStorageModeShared];
                                     ...
                                     bool refit)
{
  ...
  for (size_t step = 0; step < num_motion_steps; ++step) {
    const packed_float3 *step_points = attr_P->data_at_time_step<packed_float3>(step,
                                                                                num_motion_steps);
...
#  endif
  ...
  for (Object *ob : objects) {
    ...
    if (geom->is_hair()) {
      ...
      PointCloud *const pointcloud = static_cast<PointCloud *const>(const_cast<Geometry *>(geom));
