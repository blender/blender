...
#    if defined(MAC_OS_VERSION_13_3)
        if (@available(macOS 13.3, *)) {
          ...
          max_mtlcompiler_threads = max(
              2, int([mtlDevice maximumConcurrentCompilationTaskCount]) - 1);
        }
#    endif
#  endif
    ...
    /* Keep track of the originating device's ID so that we can cancel requests if the device
     * ceases to be active. */
...
#  endif
  ...
  MetalKernelPipeline *ShaderCache::get_best_pipeline(DeviceKernel kernel,
                                                      const MetalDevice *device)
  {
...
#  endif
  ...
  static MTLFunctionConstantValues *GetConstantValues(const KernelData *data = nullptr)
  {
    ...
    [constant_values setConstantValue:&zero_data
                                 type:MTLDataType_int
...
#  endif
    ...
    auto do_compilation = [&]() {
      ...
      if (loading_existing_archive || !DebugFlags().metal.use_async_pso_creation) {
        ...
        [mtlDevice newComputePipelineStateWithDescriptor:computePipelineStateDescriptor
                                                 options:pipelineOptions
                                       completionHandler:^(
                                         ...
                                         const char *err = error ? [[error localizedDescription]
                                                                       UTF8String] :
                                                                   nullptr;
                                         error_str = err ? err : "nil";
  ...
  bool MetalDeviceKernels::should_load_kernels(const MetalDevice *device,
                                               MetalPipelineType pso_type)
  {
