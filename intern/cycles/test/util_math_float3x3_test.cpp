...
TEST_F(Float3x3Test, quaternion_to_scaled_rotation)
{
  EXPECT_THAT(quaternion_to_scaled_rotation(
                  make_float4(2.0f * cosf(M_PI_F / 4.0f), 0, 0, 2.0f * sinf(M_PI_F / 4.0f))),
...
TEST_F(Float3x3Test, quaternion_to_rotation)
{
  ...
  EXPECT_THAT(quaternion_to_rotation(make_float4(cosf(M_PI_F / 4.0f), 0, 0, sinf(M_PI_F / 4.0f))),
...

  {
    ...
    const float3x3 R = quaternion_to_rotation(make_float4(q.w, q.x, q.y, q.z));
