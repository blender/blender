...
void encoderSetQuantizationBits(Encoder *encoder,
                                uint32_t position,
                                uint32_t normal,
                                uint32_t uv,
                                uint32_t color,
...
bool encoderEncode(Encoder *encoder, uint8_t preserveTriangleOrder)
{
  ...
  if (encoderStatus.ok()) {
    ...
    float compressionRatio = static_cast<float>(encoder->rawSize) /
                             static_cast<float>(encodedSize);
    printf(LOG_PREFIX "Encoded %" PRIu32 " vertices, %" PRIu32
                      " indices, raw size: %zu, encoded size: %zu, compression ratio: %.2f\n",
...
template<class T> void encodeIndices(Encoder *encoder, uint32_t indexCount, T *indices)
{
  ...
  for (int i = 0; i < face_count; ++i) {
    draco::Mesh::Face face = {draco::PointIndex(indices[3 * i + 0]),
...
void encoderSetIndices(Encoder *encoder,
                       size_t indexComponentType,
                       uint32_t indexCount,
                       void *indices)
{
                    ...
                    bool normalized)
{
  ...
  attribute.Init(
      semantics, &*buffer, componentCount, getDataType(componentType), normalized, stride, 0);
  ...
  for (uint32_t i = 0; i < count; i++) {
    encoder->mesh.attribute(id)->SetAttributeValue(draco::AttributeValueIndex(i),
                                                   dataBytes + i * stride);
  }
